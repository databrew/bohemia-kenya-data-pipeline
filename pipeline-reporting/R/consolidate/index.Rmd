...
odk_url_access <- 'https://databrew.org/-/single/CLd8IzvVNB5pfAm8QZ6aIftTt5r2IO9?st=D9fnhK7iSUaYIhf5$sTRmQS8xmAjnZr2ouvz!45FgU4OjrXLiC4sSyidRjaXjwry'
```
...
```{r}
reactable(
    data %>% dplyr::filter(topic != 'monitoring'),
    columns = list(
...
3. To provide monitoring review, click the URL under `ODK Review URL` where it will pre-fill ODK Forms with the report ID you need
...
data <- cloudbrewr::aws_s3_get_catalog(bucket = bucket_lake_db) %>% 
  ...
  dplyr::mutate(report_name = basename(key)) %>%
    ...
    dplyr::select(report_name, 
                  ...
                  topic = folder) %>%
    dplyr::ungroup() %>% 
    dplyr::mutate(odk_url_key = glue::glue('{odk_url_access}&d%5B/data/group_indentifier/report_id%5D={report_id}',
...
reactable(
    ...
    columns = list(
        ...
        odk_url_key = colDef(name = 'ODK Review URL',
                         cell = function(value, index) {
                           url <- sprintf("%s", 
                                          data[index, "odk_url_key"], value)
                           htmltools::tags$a(href = url, target = "_blank", 'URL')
                        })
    ),
    groupBy = "topic",
