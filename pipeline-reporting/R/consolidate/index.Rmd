...
data <- cloudbrewr::aws_s3_get_catalog(bucket = bucket_lake_db) %>%
  ...
  dplyr::mutate(report_name = basename(key)) %>%
    ...
    dplyr::select(report_name, size, last_modified, url_key, folder) %>%
    split(.$folder)
```

### Anomalies
```{r}
create_reactable(data$anomalies)
```

### V0-Demography
```{r}
create_reactable(data$`v0-demography`)
```

### Ento
```{r}
create_reactable(data$ento)
```

### ICF
```{r}
create_reactable(data$icf)
```
