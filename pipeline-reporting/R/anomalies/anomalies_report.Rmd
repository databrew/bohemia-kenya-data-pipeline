...
data_list$total <- anomalies %>% 
  dplyr::summarise(anomalies = n_distinct(resolution_id))

data_list$resolved <- fully_resolved %>% 
  dplyr::summarise(`Resolved` = n_distinct(resolution_id))

data_list$pending <- pending_resolution %>% 
  dplyr::summarise(`Pending Resolution` = n_distinct(resolution_id))

reactable(data_list %>% purrr::reduce(dplyr::bind_cols))
```
