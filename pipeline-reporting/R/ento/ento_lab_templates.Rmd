...
parse_to_tubes <- function(data, repeat_target, col_target) {
  data %>% 
    dplyr::select(KEY, SubmissionDate, 
                  test_mosquitoes_age, 
                  num_mosquitoes_exposed, 
                  starts_with('num_alive'), starts_with('num_dead')) %>% 
    dplyr::filter(!!sym(col_target) > 0) %>%
    dplyr::inner_join(rep_list[[repeat_target]], by = c('KEY' = 'PARENT_KEY')) %>% 
    dplyr::group_by(KEY, SubmissionDate, test_mosquitoes_age, num_mosquitoes_exposed) %>% 
    dplyr::mutate(sequence = row_number() -1) %>% 
    dplyr::mutate(sequence = sequence * 10) %>% 
    dplyr::mutate(mosq_in_tube := !!sym(col_target)) %>%
    dplyr::mutate(mosq_in_tube = 10) %>% 
    dplyr::select(KEY, 
                  SubmissionDate, 
                  test_mosquitoes_age, 
                  ends_with('tube_qr'), 
...
ento_labs_template_entoinsecticideresistance <- data_list %>%
    ...
    dplyr::mutate(mosq_number = mosq_number - 1) %>%
    dplyr::mutate(mosq_number_str = glue("{sprintf('%02d', mosq_number)}")) %>% 
    dplyr::ungroup() %>%
    dplyr::mutate(mosquito_id = glue::glue('{tube_qr}-{mosq_number_str}'))
# 
# 
# create directory
output_dir <-'tmp'
...
ento_labs_template_entoinsecticideresistance %>% 
  dplyr::inner_join(entoinsecticideresistance %>% 
                      dplyr::select(KEY, 
                                    synergist, 
                                    synergist_assay, 
                                    insecticide, 
