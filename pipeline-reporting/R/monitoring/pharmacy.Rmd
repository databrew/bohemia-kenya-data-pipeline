...
1. `Missing Tablets`: `Arrival` - (`Dispatch` - `Returns (Unused)` - `Returns (Unusable / Lost)` - `Returns Empty Blisters`)
...
```{r}
tbl <- df %>% 
  dplyr::mutate(pharmacy_missing_tablets = 
                  pharmacy_arrival - (pharmacy_dispatch - 
                                        pharmacy_unused_return - 
                                        pharmacy_unusable_return - 
                                        pharmacy_empty_blister_return)) %>%
  dplyr::select(run_date, 
                ...
                `Pharmacy Missing Tablets` = pharmacy_missing_tablets
                )
...
```{r}
tbl <- df %>% 
  dplyr::mutate(sfa_missing_tablets = 
                sfa_arrival - (sfa_dispatch - 
                                        sfa_unused_return - 
                                        sfa_unusable_return - 
                                        sfa_empty_blister_return)) %>%
  dplyr::select(run_date, 
                name,
                ...
                `SFA Missing Tablets` = sfa_missing_tablets
                )
...
```{r}
tbl <- df %>% 
  dplyr::mutate(fa_missing_tablets = 
                fa_arrival - (fa_dispatch - 
                                        fa_unused_return - 
                                        fa_unusable_return - 
                                        fa_empty_blister_return)) %>%
  dplyr::select(run_date, 
                name,
                ...
                `FA Missing Tablets` = fa_missing_tablets,
