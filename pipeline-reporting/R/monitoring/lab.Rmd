...
### c). Missing / Waiting to be logged into lab database Samples
```{r}
...
reactable(tbl, 
          groupBy = c('run_date'),
          columns = list(
            `Missing Samples` = colDef(aggregate = "sum"),
            `Missing Samples 0-10 days` = colDef(aggregate = "sum"),
            `Missing Samples 10-20 days` = colDef(aggregate = "sum"),
            `Missing Samples >20 days` = colDef(aggregate = "sum")
          ),
          defaultSorted=list(run_date = 'desc'))
```
