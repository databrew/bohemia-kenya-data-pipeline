...
# create the dataset that will be used for this script
  ...
  data <- merge(pfu_reason, v0dem, by = c('extid'), all = TRUE) # add manual changes with filter function here 
...
## create dataset to determine if it was an enrolment or follow up visit
visit_type <- data %>%
    pivot_wider(
    id_cols = extid,  # Incorrect column name
    names_from = visit,
    values_from = pregnancy_status
  ) %>% 
  select(extid, V2, V3) %>% # add V4, etc.
  filter(!is.na(V2) | !is.na(V3)) # add V4, etc.

# add the visit type column to data
# need to make the visit type show and need to break up eos into visit type options

# ## V4 #########################
# 
# visit_type_v4 <- visit_type %>%
#   filter(!is.na(V4)) %>%
...
#       is.na(V3) ~ 'enrolment',
#     V2 == 'eos' |
#       V3 == 'eos' ~ 'previously eos'
#   ))
# 
#   table_visit_type_v4 <- visit_type_v4 %>%
#       group_by(visit_type) %>%
#       summarise('Number of people' = n()) %>%
#       arrange(visit_type) %>%
#       rename('Visit type' = visit_type) %>%
#       add_row('Visit type' = 'total visited', 'Number of people' = sum(.$'Number of people')) %>%
#       mutate('Percent complete' = (as.numeric(`Number of people`)) / as.numeric(nrow(visit_type_v4)) * 100) %>%
#       mutate('Percent complete' = round(`Percent complete`, 2)) %>%
#       mutate('Percent complete' = paste0(`Percent complete`, '%')) # Replace zero values with NA

## V3 ######################### 
  visit_type_v3 <- visit_type %>%
      filter(!is.na(V3)) %>%
      mutate(visit_type = case_when(
        V2 == 'in' ~ 'follow up',
        V2 == 'out' |
          is.na(V2) ~ 'enrolment',
        V2 == 'eos' ~ 'previously eos'
    ))
  
    table_visit_type_v3 <- visit_type_v3 %>%
      group_by(visit_type) %>%
      summarise('Number of people' = n()) %>%
      arrange(visit_type) %>%
      rename('Visit type' = visit_type) %>%
      add_row('Visit type' = 'total visited', 'Number of people' = sum(.$'Number of people')) %>%
      mutate('Percent complete' = (as.numeric(`Number of people`)) / as.numeric(nrow(visit_type_v3)) * 100) %>%
      mutate('Percent complete' = round(`Percent complete`, 2)) %>%
      mutate('Percent complete' = paste0(`Percent complete`, '%')) # Replace zero values with NA
```
