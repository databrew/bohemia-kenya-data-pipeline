...
pfu_eos_v2 <- pfu_v2 %>%
  select(extid, hhid, cluster, ltfu_eos, person_died_eos, person_migrated_eos, fourth_absence_eos, not_continue_eos, still_preg_no_eos, still_preg_no_reason) %>%
...
if (length(not_found_names) > 0) {
  cat("Note: The reasons like (", paste0(not_found_names, collapse = " / "), ") are not shown because it is not a reason for anyone being eos.")
}
```