...
This report is used for SE & PFU on this [Reporting Document](https://docs.google.com/spreadsheets/d/1yzrP80EjjPtTan3Dl_rSoQOUu1sARKHj9hd1k4-nPc4/edit#gid=1969855170)
...
get_target_by_assignment <- function(visit_list){
  # create targets
  v0_target <- v0 %>% 
...
get_target_by_cluster_and_village <- function(visit_list){
    v0_target <- v0 %>% 
      dplyr::inner_join(v0_repeat, by = c('KEY' = 'PARENT_KEY')) %>%
...
target_df_by_assignment = get_target_by_assignment(visit_list =  unique_visit_list)
target_df_by_cluster_and_village = get_target_by_cluster_and_village(visit_list =  unique_visit_list)
...
data_list$safety_overall <- prep_safety_tbl(
    ...
    target_df = target_df_by_assignment,
...
```{r}
summary <- target_df_by_assignment %>%
...
data_list$safety_overall <- prep_safety_tbl(
    ...
    safetynew_df = safetynew_merged_tbl, 
    indicator = 'safety_overall',
    target_df = target_df_by_cluster_and_village,
    group_cols = group_cols,
    visit_list = unique_visit_list
)
...
summary <- target_df_by_cluster_and_village %>%
    dplyr::left_join(purrr::reduce(data_list, dplyr::bind_rows), by = group_cols) %>%
    tidyr::pivot_wider(names_from = metric, 
                       id_cols = c(any_of(group_cols), 'hh_target', 'ind_target'),
                       values_from = c('ind','hh')) %>% 
...
tbl <- reactable(
    ...
    columns = list(
        ...
        village = colDef(name = 'Village', filterable = TRUE, minWidth = 200),
        hh_target = colDef(name = "Target Household"),
        ind_target = colDef(name = "Target Individual"),
        hh_safety_overall = colDef(name = "Household Visits"),
        ind_safety_overall = colDef(name = "Individual Visits"),
...
wrap_download(
  tbl, 
  element_id,
  'safety_efficacy_target_by_cluster_and_village.csv')
```
