...
# village mapping
v0_mapping <- v0_merged_tbl %>% 
    dplyr::select(hhid, cluster = geo_cluster_num, assignment, village) %>% 
    distinct()
...
safety_merged_tbl <- safety %>% 
  ...
  dplyr::inner_join(v0_mapping, by = 'hhid') %>% 
...
attendance <- dplyr::bind_rows(
  safety %>% 
    dplyr::select(wid, instanceID, SubmissionDate, hhid, visit, cluster) %>% 
