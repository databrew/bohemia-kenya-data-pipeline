...
get_safety_target_by_assignment <- function(visit_list){
  ...
  # available safety ind members
  avail_ind_members <- v0_merged_tbl %>% 
    dplyr::select(extid, hhid) %>% 
    dplyr::group_by(hhid) %>% 
    dplyr::summarise(avail_ind_members = n_distinct(extid))
  
  # get safety eos
  ind_eos <- safety_merged_tbl %>% 
      dplyr::select(visit, hhid, extid, safety_status) %>%
      dplyr::filter(safety_status == 'eos') %>% 
      dplyr::inner_join(v0_merged_tbl %>% dplyr::select(extid), 
                        by = 'extid') %>% 
      dplyr::group_by(visit, hhid) %>%
      dplyr::summarise(ind_eos = n_distinct(extid))
  
  # create targets
  remove_from_next_target <- ind_eos %>% 
    dplyr::inner_join(avail_ind_members) %>% 
    dplyr::mutate(remaining_member = avail_ind_members - ind_eos) %>% 
    dplyr::inner_join(v0_merged_tbl %>% 
                          dplyr::select(hhid, assignment) %>% 
                          dplyr::distinct(), 
                      by = 'hhid') %>%
    dplyr::group_by(visit, assignment) %>% 
    dplyr::summarise(ind_rm = sum(ind_eos),
                     hh_rm = n_distinct(hhid[remaining_member == 0])) %>%
    dplyr::ungroup() %>% 
    dplyr::mutate(
...
get_safety_target_by_cluster_and_village <- function(visit_list){
  ...
  remove_from_next_target <- ind_eos %>% 
    ...
    dplyr::mutate(remaining_member = avail_ind_members - ind_eos) %>% 
    dplyr::inner_join(v0_merged_tbl %>% 
                          dplyr::select(hhid, cluster, village) %>% 
                          dplyr::distinct(), 
                      by = 'hhid') %>%
...
get_efficacy_target_by_assignment <- function(visit_list){
  ...
  # available efficacy ind members
  avail_ind_members <- v0_merged_tbl %>% 
      dplyr::inner_join(
        efficacy_selection, 
        by = c('extid')) %>%
  ...
  # get efficacy eos
  ind_eos <- efficacy %>% 
      dplyr::select(visit, hhid, extid, efficacy_status) %>%
...
get_efficacy_target_by_cluster_and_village <- function(visit_list){
  ...
  # get summary of user and household to remove next visit
  remove_from_next_target <- ind_eos %>% 
    dplyr::inner_join(avail_ind_members) %>% 
    dplyr::mutate(remaining_member = avail_ind_members - ind_eos) %>% 
    dplyr::inner_join(v0_merged_tbl %>% dplyr::select(hhid, cluster = geo_cluster_num, village), 
                      by = 'hhid') %>%
    dplyr::group_by(visit, cluster, village) %>% 
    dplyr::summarise(ind_rm = sum(ind_eos),
                     hh_rm = n_distinct(hhid[remaining_member == 0])) %>%
