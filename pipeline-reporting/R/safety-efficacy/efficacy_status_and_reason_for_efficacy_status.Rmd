...
```{r}
data_visit_type <- data_investigation %>%
  pivot_wider(
    id_cols = extid,
    names_from = visit,
    values_from = efficacy_status
  ) %>%
  select(extid, V1, V2, V3, V4) 
```
...
## V5 #########################
  efficacy_v5 <- total_efficacy_v5 %>%
    rename(v5_efficacystatus = efficacy_status,
           v5_efficacyreason = efficacy_reason) %>%
    select(extid, v5_efficacystatus, v5_efficacyreason) # get the v2 'in' people
  
  efficacy_v5 <- left_join(efficacy_v5, total_efficacy_v4, by=c('extid')) %>%
    select(extid, efficacy_status, v5_efficacystatus, v5_efficacyreason) %>%
    rename(v4_efficacystatus = efficacy_status) %>% # join the v2 'in' people with their v1 statuses
    mutate(visit_type = case_when( # create a column that says whether the safety people are follow up or enrolment.
      v4_efficacystatus == 'in' ~ 'follow up',
      v4_efficacystatus == 'out' ~ 'enrolment',
        is.na(v4_efficacystatus) ~ 'follow up but missed previous visit'
    ))
  
  efficacy_v5_in <- efficacy_v5 %>%
    filter(v5_efficacystatus == 'in')
...
## V4 #########################
  ...
  efficacy_v4 <- left_join(efficacy_v4, total_efficacy_v3, by=c('extid')) %>%
    ...
    mutate(visit_type = case_when( # create a column that says whether the safety people are follow up or enrolment.
      ...
      v3_efficacystatus == 'out' ~ 'enrolment',
        is.na(v3_efficacystatus) ~ 'follow up but missed previous visit'
    ))
...
# create datasets to make an enrolment and follow up eos table
  total_eff_eos_missed_fu_v4 <- efficacy_eos_v4 %>%
    filter(visit_type == 'follow up but missed prev visit')
  total_eff_eos_followup_v4 <- efficacy_eos_v4 %>%
    filter(visit_type == 'follow up')
  
# make a table of enrolment people who are eos
  eos_reasons_enrolment_v4 <- total_eff_eos_missed_fu_v4 %>%
    group_by(v4_efficacyreason) %>%
    summarise('Number of eos people' = n()) %>%
    arrange(v4_efficacyreason) %>%
    rename('Reason for eos' = v4_efficacyreason) %>%
    add_row('Reason for eos' = 'total visited', 'Number of eos people' = sum(.$'Number of eos people')) %>%
    mutate('Percent complete' = (as.numeric(`Number of eos people`)) / as.numeric(nrow(total_eff_eos_missed_fu_v4)) * 100) %>%
    mutate('Percent complete' = round(`Percent complete`, 2)) %>%
    mutate('Percent complete' = paste0(`Percent complete`, '%')) # Replace zero values with NA
...
## V5 ###########################
  ...
  efficacy_eos_v5 <- efficacy_v5 %>%
    filter(v5_efficacystatus == 'eos')
  
  # make a table of why people are eos
  eos_reasons_v5 <- efficacy_eos_v5 %>%
    group_by(v5_efficacyreason) %>%
    summarise('Number of eos people' = n()) %>%
    arrange(v5_efficacyreason) %>%
    rename('Reason for eos' = v5_efficacyreason) %>%
    add_row('Reason for eos' = 'total visited', 'Number of eos people' = sum(.$'Number of eos people')) %>%
...
# create datasets to make an enrolment and follow up eos table
  total_eff_eos_missed_fu_v5 <- efficacy_eos_v5 %>%
    filter(visit_type == 'follow up but missed prev visit')
  total_eff_eos_followup_v5 <- efficacy_eos_v5 %>%
    filter(visit_type == 'follow up')
  
# make a table of enrolment people who are eos
  eos_reasons_enrolment_v5 <- total_eff_eos_missed_fu_v5 %>%
    group_by(v5_efficacyreason) %>%
    summarise('Number of eos people' = n()) %>%
    arrange(v5_efficacyreason) %>%
    rename('Reason for eos' = v5_efficacyreason) %>%
    add_row('Reason for eos' = 'total visited', 'Number of eos people' = sum(.$'Number of eos people')) %>%
    mutate('Percent complete' = (as.numeric(`Number of eos people`)) / as.numeric(nrow(total_eff_eos_missed_fu_v5)) * 100) %>%
    mutate('Percent complete' = round(`Percent complete`, 2)) %>%
    mutate('Percent complete' = paste0(`Percent complete`, '%')) # Replace zero values with NA

# make a table of reasons follow up people are eos #
  eos_reasons_followup_v5 <- total_eff_eos_followup_v5 %>%
    group_by(v5_efficacyreason) %>%
    summarise('Number of eos people' = n()) %>%
    arrange(v5_efficacyreason) %>%
    rename('Reason for eos' = v5_efficacyreason) %>%
    add_row('Reason for eos' = 'total visited', 'Number of eos people' = sum(.$'Number of eos people')) %>%
    mutate('Percent complete' = (as.numeric(`Number of eos people`)) / as.numeric(nrow(total_eff_eos_followup_v5)) * 100) %>%
    mutate('Percent complete' = round(`Percent complete`, 2)) %>%
    mutate('Percent complete' = paste0(`Percent complete`, '%')) # Replace zero values with NA
...
#### follow up visit
```{r v5_eos_followup_show}

reactable(eos_reasons_followup_v5, columns = list(
  `Percent complete` = colDef(align = 'right')
), pagination = FALSE) #pagination makes the entire table show on one page
