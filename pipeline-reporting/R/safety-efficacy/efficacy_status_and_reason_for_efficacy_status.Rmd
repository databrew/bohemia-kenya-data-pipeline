...
# create the dfset that will be used for this script
  # this will exclude the people who were manually removed
  df <- left_join(total_eff, v0dem, by = c('extid')) %>%
    filter(extid != '74052-06', # dob change led to not efficacy eligible (aka out of 5-15 years)
           extid != '18039-03', # "
           extid != '02042-03', # "
           extid != '02042-02', # "
           extid != '26007-03', # "
           extid != '56118-06') # these extids have been removed from the df
  
# create the dfset for the extid explorer
  df_investigation <- df %>%
    filter(!is.na(visit)) %>% # removes extids not in Efficacy
    select(visit, extid, efficacy_status, enrolment, efficacy_reason, sex_v0, sex_efficacy, dob_v0, dob_efficacy, todays_date_v0, todays_date_efficacy, assignment)
  
  #df <- df %>% distinct(extid, visit, .keep_all = TRUE)
  ...
  by_visit <- df %>%
    ...
    select(extid, hhid, efficacy_status_V1, efficacy_status_V2, efficacy_status_V3,
           efficacy_status_V4, efficacy_status_V5, efficacy_status_V6, efficacy_status_V7,
           efficacy_reason_V1, efficacy_reason_V2, efficacy_reason_V3, 
           efficacy_reason_V4, efficacy_reason_V5, efficacy_reason_V6, efficacy_reason_V7, 
          assignment, cluster) %>%
    rename(status_V1 = efficacy_status_V1,
           ...
           status_V6 = efficacy_status_V6,
           reason_V6 = efficacy_reason_V6,
           status_V7 = efficacy_status_V7,
           reason_V7 = efficacy_reason_V7)
  
  # a trial profile tracks where every person went
  by_path <- df %>%
    pivot_wider(id_cols = c(extid, hhid, cluster, assignment), 
                names_from = visit,
                values_from = c(starting_efficacy_status, efficacy_status)) %>%
    select(extid, hhid, efficacy_status_V1, starting_efficacy_status_V1, efficacy_status_V2, starting_efficacy_status_V2, efficacy_status_V3, starting_efficacy_status_V3, efficacy_status_V4, starting_efficacy_status_V4, efficacy_status_V5, starting_efficacy_status_V5, efficacy_status_V6, starting_efficacy_status_V6, efficacy_status_V7, starting_efficacy_status_V7,  
          assignment, cluster) 
```
...
v1 <- total_eff %>% 
  filter(visit == 'V1') %>% 
  select(extid, hhid, visit, starting_efficacy_status, efficacy_status, efficacy_reason)
...
v1_removed_extids <- length(unique(v1_removed_extid_df$extid))

v1_removed_extid_table <- v1_removed_extid_df %>%
  arrange(visit) %>%
...
v1_clean <- v1 %>%
  anti_join(v1_removed_extid_df, by = "extid") %>%
...
# make a table of out and in children of those visited and target
v1_out_in_table_visited <- v1 %>% 
...
v1_out_in_table_target <- v1_clean %>% 
  group_by(efficacy_status) %>% 
  summarise(Target = n()) %>% 
  add_row(efficacy_status = 'Total',
          Target = sum(.$Target)) %>% 
...
v1_status_table <- left_join(v1_out_in_table_visited, v1_out_in_table_target, by = 'Efficacy status') %>% 
  mutate(Target = ifelse(is.na(Target) == 'TRUE', '0', Target))
...
v2_removed_extid_df <- v2 %>%
  filter(efficacy_status %in% c('refusal', 'eos') | efficacy_reason %in% c('migrated', 'died')) %>%
  mutate(removal_reason = paste(efficacy_status, efficacy_reason, sep = " "))

v2_removed_extids <- length(unique(v2_removed_extid_df$extid))

v2_removed_extid_table <- v2_removed_extid_df %>%
  arrange(visit) %>%
  group_by(removal_reason) %>%
  summarise('N' = n()) %>%
  arrange(removal_reason) %>%
...
# add the missed extids back in and remove the 100 refusal houses
v2_clean <- v2 %>%
  anti_join(v2_removed_extid_df, by = "extid") %>%
...
v2_out_in_table_target <- v2_clean %>% 
  group_by(efficacy_status) %>% 
  summarise(Target = n()) %>% 
  add_row(efficacy_status = 'Total',
          Target = sum(.$Target)) %>% 
...
v2_status_table <- left_join(v2_out_in_table_visited, v2_out_in_table_target, by = 'Efficacy status') %>% 
  mutate(Target = ifelse(is.na(Target) == 'TRUE', '0', Target))
...
v3 <- df %>% 
  filter(visit == 'V3') %>% 
  select(extid, hhid, visit, starting_efficacy_status, efficacy_status, efficacy_reason, enrolment)
...
v3_removed_extids <- length(unique(v3_removed_extid_df$extid))

v3_removed_extid_table <- v3_removed_extid_df %>%
  arrange(visit) %>%
  group_by(removal_reason) %>%
  summarise('N' = n()) %>%
  arrange(removal_reason) %>%
...
v3_never_enrolled <- v3_removed_extid_df %>%
...
v3_clean <- v3 %>%
  anti_join(v3_removed_extid_df, by = "extid") %>%
...
v3_out_in_table_target <- v3_clean %>% 
  group_by(efficacy_status) %>% 
  summarise(Target = n()) %>% 
  add_row(efficacy_status = 'Total',
          Target = sum(.$Target)) %>% 
...
v3_status_table <- left_join(v3_out_in_table_visited, v3_out_in_table_target, by = 'Efficacy status') %>% 
  mutate(Target = ifelse(is.na(Target) == 'TRUE', '0', Target))
...
v4_removed_extid_df <- v4 %>%
  filter(efficacy_status %in% c('refusal', 'eos') | efficacy_reason %in% c('migrated', 'died')) %>%
  mutate(removal_reason = paste(efficacy_status, efficacy_reason, sep = " "))

v4_removed_extids <- length(unique(v4_removed_extid_df$extid))

v4_removed_extid_table <- v4_removed_extid_df %>%
  arrange(visit) %>%
  group_by(removal_reason) %>%
  summarise('N' = n()) %>%
  arrange(removal_reason) %>%
...
# add the missed extids back in and remove the 100 refusal houses
v4_clean <- v4 %>%
  anti_join(v4_removed_extid_df, by = "extid") %>%
...
v5_removed_extid_df <- v5 %>%
  filter(efficacy_status %in% c('refusal', 'eos') | efficacy_reason %in% c('migrated', 'died')) %>%
  mutate(removal_reason = paste(efficacy_status, efficacy_reason, sep = " "))

v5_removed_extids <- length(unique(v5_removed_extid_df$extid))

v5_removed_extid_table <- v5_removed_extid_df %>%
  arrange(visit) %>%
  group_by(removal_reason) %>%
  summarise('N' = n()) %>%
  arrange(removal_reason) %>%
...
# add the missed extids back in and remove the 100 refusal houses
v5_clean <- v5 %>%
  anti_join(v5_removed_extid_df, by = "extid") %>%
...
v6_removed_extid_df <- v6 %>%
  filter(efficacy_status %in% c('refusal', 'eos') | efficacy_reason %in% c('migrated', 'died')) %>%
  mutate(removal_reason = paste(efficacy_status, efficacy_reason, sep = " "))

v6_removed_extids <- length(unique(v6_removed_extid_df$extid))

v6_removed_extid_table <- v6_removed_extid_df %>%
  arrange(visit) %>%
  group_by(removal_reason) %>%
  summarise('N' = n()) %>%
  arrange(removal_reason) %>%
...
# add the missed extids back in and remove the 100 refusal houses
v6_clean <- v6 %>%
  anti_join(v6_removed_extid_df, by = "extid") %>%
...
v7_removed_extid_df <- v7 %>%
  filter(efficacy_status %in% c('refusal', 'eos') | efficacy_reason %in% c('migrated', 'died')) %>%
  mutate(removal_reason = paste(efficacy_status, efficacy_reason, sep = " "))

v7_removed_extids <- length(unique(v7_removed_extid_df$extid))

v7_removed_extid_table <- v7_removed_extid_df %>%
  arrange(visit) %>%
  group_by(removal_reason) %>%
  summarise('N' = n()) %>%
  arrange(removal_reason) %>%
...
# add the missed extids back in and remove the 100 refusal houses
v7_clean <- v7 %>%
  anti_join(v7_removed_extid_df, by = "extid") %>%
...
#write.csv(status_pathway, file='V2.0 repeat efficacy status pathways.csv')

```


```{r enrolled}
# create a dfset of enrolled participants (people who became efficacy status in)
  enrolled <- df %>%
  ...
  enrol_in <- by_visit %>%
    filter(extid %in% enrolled$extid,
           !(extid %in% enrol_eos_df$extid),
           status_V7 == 'completion' | is.na(status_V7))
  ...
  enrol_in_table <- enrol_in_df %>%
    ...
    pivot_wider(names_from = assignment, values_from = N, values_fill = 0) %>%
    ungroup() %>%
    rename('Enrolled completion' = efficacy_status) %>%
    mutate(Total = `1` + `2`) %>%
    rename('Assignment 1' = '1',
           'Assignment 2' = '2')
  
  merged <- merge(enrol_eos_df, enrol_in, by='extid', all = TRUE)
...
# not enrolled participants
  ...
  not_enrolled_table <- not_enrolled_df %>%
    arrange(visit) %>%
    group_by(efficacy_status, assignment) %>%
    summarise('N' = n()) %>%
    group_by(efficacy_status) %>%
...
# EOS
  ...
  not_enrol_eos_table <- not_enrol_eos_df %>%
    arrange(visit) %>%
    group_by(efficacy_reason, assignment) %>%
    summarise('N' = n()) %>%
    group_by(efficacy_reason) %>%
...
# OUT
  ...
  not_enrol_out_df <- df %>%
    ...
    slice(1) %>%
    ungroup() %>% 
    select(extid, visit, starting_efficacy_status, efficacy_status, efficacy_reason, assignment)
  
  not_enrol_out_table <- not_enrol_out_df %>%
    arrange(visit) %>%
    group_by(efficacy_reason, assignment) %>%
    summarise('N' = n()) %>%
    group_by(efficacy_reason) %>%
...
# REFUSAL
  ...
  not_enrol_ref_table <- not_enrol_ref_df %>%
    arrange(visit) %>%
    group_by(efficacy_reason, assignment) %>%
    summarise('N' = n()) %>%
    group_by(efficacy_reason) %>%
...
# create dfsets to make an enrolment and follow up eos table
...
# present the rdt results for the different visits
  eff_results <- df %>%
    select(extid, cluster, pan_result, pf_result, visit, starting_efficacy_status, efficacy_status, efficacy_reason, assignment, desiccant_color, desiccant_color_2, rdt_control_window, rdt_control_window_b) %>%
...
After removing migrations, deaths, refusals, and eos from above, we have the following in and out children:

```{r}
knitr::kable(v1_status_table)
```


And we enter V2 with a target of `r num_v1_clean`. See below:
...
```{r}
knitr::kable(v2_status_table)
```
_NOTE: `r length(unique(status_pathway$extid))` extids were found to have transitioned 'out to in' in both V1 and V2, which is a mistake and was not corrected in the data._

And we enter V3 with a target of `r num_v2_clean`. See below:
...
This number discrepancy can be explained by the following:
  ...
  * `r num_v3_not_visited` children were not visited in V3 (aka missed the visit). _Note: only `r num_v3_not_visited_keep` of the missed children was efficacy status 'in' and the rest were efficacy status out._  
...
```{r}
knitr::kable(v3_status_table)
```


And we enter V4 with a target of `r num_v3_clean`. See below:

  * `r v3_total_visited` (V3 submissions) + `r num_v3_not_visited_keep` (missed extid who is efficacy status 'in') - `r v3_removed_extids` (removed extids) = `r  sum(v3_total_visited + num_v3_not_visited_keep - v3_removed_extids)`
...
Now, we will determine the V5 target.
...
Now we will determine the V6 target.
...
Now we will determine the V7 target.
...
## Flowcharts
Flowcharts are currently under review.

### Arm 1
```{r flowchart_assignment_1}

# number of efficacy preselected children per arm
efficacy_selected_1 <- df %>% 
  filter(extid %in% efficacy_selection$extid, 
         # the efficacy selection list still contains children who aged out
         assignment==1) %>% 
  summarise(efficacy_selected_1 = n_distinct(extid)) %>% 
  pull(efficacy_selected_1)

# calculate children visited
extid_visited_1 <- df %>%
  filter(!is.na(visit), 
         assignment == 1) %>%
  summarise(extid_visited_1 = n_distinct(extid)) %>%
  pull(extid_visited_1)
...
non_enrolled_1 <- not_enrolled %>%
  ...
  # not eligible (children who did not pass inclusion criteria. the inclusion criteria is to be a resident and not be in any other trials)
  not_enrol_eligible_1 <- not_enrolled %>% 
    filter(assignment == 1,
           !extid %in% not_enrol_ref_1$extid,
           !extid %in% not_enrol_mig_died_1$extid,
           reason_V1 %in% c('not a resident', 'enrolled in other trials') 
             & status_V1 == 'eos' | 
             reason_V2 %in% c('not a resident', 'enrolled in other trials') 
           & status_V2 == 'eos' | 
    reason_V3 %in% c('not a resident', 'enrolled in other trials') & status_V3 == 'eos')
  ...
  not_enrol_other_1 <- not_enrolled %>% 
    filter(assignment == 1,
           ...
           (reason_V1 %in% c('not agree to efficacy procedures', 
                             'decline participation') |
          reason_V2 %in% c('not agree to efficacy procedures', 
                             'decline participation') |
          reason_V3 %in% c('not agree to efficacy procedures', 
                             'decline participation') |
          reason_V4 %in% c('not agree to efficacy procedures', 
                             'decline participation') |
          reason_V5 %in% c('not agree to efficacy procedures', 
                             'decline participation') |
          reason_V6 %in% c('not agree to efficacy procedures', 
                             'decline participation') |
          reason_V7 %in% c('not agree to efficacy procedures', 
                             'decline participation')))
...
# calculate visited per visit 
# calculate valid test per visit
# calculate invalid test per visit
# calculate number of absences per visit

  enrolled_tested_v1_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==1,
           visit == 'V1') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v1_1 <- eff_results %>% 
    filter(visit == 'V1',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
           (desiccant_color == 'Blue' | desiccant_color_2 == 'Blue') &
             (rdt_control_window == 'b' | rdt_control_window_b == 'b') &
             combination != 'blank test responses') %>% 
    distinct(extid) %>% 
    nrow()
  
  
  enrolled_invalid_v1_1 <- eff_results %>% 
    filter(visit == 'V1',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
           ((desiccant_color %in% c('Pink', 'Colorless') & 
              desiccant_color_2 %in% c('Pink', 'Colorless')) |
              (rdt_control_window == 'a' & rdt_control_window_b == 'a')),
           combination == 'blank test responses') %>% 
    distinct(extid) %>% 
    nrow()
  
  enrol_absent_v1_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==1,
           visit == 'V1',
           efficacy_reason == 'absent') %>% 
    distinct(extid) %>% 
    nrow()
  
# v2
enrolled_tested_v2_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment == 1,
           visit == 'V2') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v2_1 <- eff_results %>% 
    filter(visit == 'V2',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v2_1 <- eff_results %>% 
    filter(visit == 'V2',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
    ...
    distinct(extid) %>% 
    nrow()
  # remember here that the number of Valid test and not took drug is only looking at in people so the others are not accounted for
  
  enrol_absent_v2_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==1,
           visit == 'V2',
           efficacy_reason == 'absent') %>% 
...
# v3
  enrolled_tested_v3_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==1,
           visit == 'V3') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v3_1 <- eff_results %>% 
    filter(visit == 'V3',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
    ...
    distinct(extid) %>% 
    nrow()
  # this number seems to be off because 07063-03 has a dessicant color of blue but a rdt panel of a. Also this person was ltfu but is still in? 
  
  
  enrolled_invalid_v3_1 <- eff_results %>% 
    filter(visit == 'V3',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v4
  enrolled_tested_v4_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==1,
           visit == 'V4') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v4_1 <- eff_results %>% 
    filter(visit == 'V4',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v4_1 <- eff_results %>% 
    filter(visit == 'V4',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v5  
  enrolled_tested_v5_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==1,
           visit == 'V5') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v5_1 <- eff_results %>% 
    filter(visit == 'V5',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v5_1 <- eff_results %>% 
    filter(visit == 'V5',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v6
  ...
  enrolled_tested_v6_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==1,
           visit == 'V6') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v6_1 <- eff_results %>% 
    filter(visit == 'V6',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v6_1 <- eff_results %>% 
    filter(visit == 'V6',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v7
  enrolled_tested_v7_1 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==1,
           visit == 'V7') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v7_1 <- eff_results %>% 
    filter(visit == 'V7',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'completion',
  ...
  enrolled_invalid_v7_1 <- eff_results %>% 
    filter(visit == 'V7',
           assignment == 1,
           extid %in% enrolled$extid,
           efficacy_status == 'completion',
  ...
  # do not include absences here because they are counted in the eos section 
...
flowchart_1 <- glue("
  digraph flowchart {{
    ...
    preselected [label = '{efficacy_selected_1} pre-selected children'];
    extids_visited [label = '{extid_visited_1} children visited'];
    non_enrolled [label = '{non_enrolled_1} non-enrolled children'];
    non_enrolled_reasons [label = 'Absent {num_not_enrol_absent_1}\nMigrated/Died {num_not_enrol_mig_died_1}\nRefusal {num_not_enrol_ref_1}\nNot eligible {num_not_enrol_eligible_1}\nOther {num_not_enrol_other_1}'];
    enrolled [label = '{enrolled_1} enrolled subjects'];
    v1 [label = '{enrolled_tested_v1_1} visited in V1\nValid test in V1: {enrolled_valid_v1_1}\nInvalid test in V1: {enrolled_invalid_v1_1}\nAbsent in V1: {enrol_absent_v1_1}'];
    v2 [label = '{enrolled_tested_v2_1} visited in V2\nValid test in V2: {enrolled_valid_v2_1}\nInvalid test in V2: {enrolled_invalid_v2_1}\nAbsent in V2: {enrol_absent_v2_1}'];
    v3 [label = '{enrolled_tested_v3_1} visited in V3\nValid test in V3: {enrolled_valid_v3_1}\nInvalid test in V3: {enrolled_invalid_v3_1}\nAbsent in V3: {enrol_absent_v3_1}'];
    v4 [label = '{enrolled_tested_v4_1} visited in V4\nValid test in V4: {enrolled_valid_v4_1}\nInvalid test in V4: {enrolled_invalid_v4_1}\nAbsent in V4: {enrol_absent_v4_1}'];
    v5 [label = '{enrolled_tested_v5_1} visited in V5\nValid test in V5: {enrolled_valid_v5_1}\nInvalid test in V5: {enrolled_invalid_v5_1}\nAbsent in V5: {enrol_absent_v5_1}'];
    v6 [label = '{enrolled_tested_v6_1} visited in V6\nValid test in V6: {enrolled_valid_v6_1}\nInvalid test in V6: {enrolled_invalid_v6_1}\nAbsent in V6: {enrol_absent_v6_1}'];
    v7 [label = '{enrolled_tested_v7_1} visited in V7\nValid test in V7: {enrolled_valid_v7_1}\nInvalid test in V7: {enrolled_invalid_v7_1}\n{enrolled_comp_1} completed the study'];
    eos_fu_v2 [label = 'Early termination in V2: {enrolled_eos_fu_v2_1}'];
    eos_fu_v3 [label = 'Early termination in V3: {enrolled_eos_fu_v3_1}'];
    eos_fu_v4 [label = 'Early termination in V4: {enrolled_eos_fu_v4_1}'];
    eos_fu_v5 [label = 'Early termination in V5: {enrolled_eos_fu_v5_1}'];
    eos_fu_v6 [label = 'Early termination in V6: {enrolled_eos_fu_v6_1}'];
    eos_fu_v7 [label = 'Early termination in V7: {enrolled_eos_fu_v7_1}'];
    

    # Define edges
    preselected -> extids_visited;
    extids_visited -> non_enrolled;
    non_enrolled -> non_enrolled_reasons;
    extids_visited -> enrolled;
    enrolled -> v1;
    v1 -> v2;
    v2 -> eos_fu_v2;
    v2 -> v3;
...
# number of efficacy preselected children per arm
efficacy_selected_2 <- df %>% 
  filter(extid %in% efficacy_selection$extid,
         assignment==2) %>% 
  summarise(efficacy_selected_2 = n_distinct(extid)) %>% 
  pull(efficacy_selected_2)

# calculate children visited
extid_visited_2 <- df %>%
  filter(!is.na(visit), 
         assignment == 2) %>%
  summarise(extid_visited_2 = n_distinct(extid)) %>%
  pull(extid_visited_2)
...
# migrated or died
  ...
  # not eligible (children who did not pass inclusion criteria. the inclusion criteria is to be a resident and not be in any other trials)
  not_enrol_eligible_2 <- not_enrolled %>% 
    filter(assignment == 2,
...
enrolled_2 <- enrolled %>%
  ...
  enrolled_tested_v1_2 <- eff_results_v1_clean %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V1') %>% 
    distinct(extid) %>% 
  ...
  enrolled_tested_v2_2 <- eff_results_v2_clean %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V2') %>% 
    distinct(extid) %>% 
  ...
  enrolled_tested_v3_2 <- eff_results_v3_clean %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V3') %>% 
    distinct(extid) %>% 
  ...
  enrolled_tested_v4_2 <- eff_results_v4_clean %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V4') %>% 
    distinct(extid) %>% 
  ...
  enrolled_tested_v5_2 <- eff_results_v5_clean %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V5') %>% 
    distinct(extid) %>% 
  ...
  enrolled_tested_v6_2 <- eff_results_v6_clean %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V6') %>% 
    distinct(extid) %>% 
  ...
  enrolled_tested_v7_2 <- eff_results_v7_clean %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V7') %>% 
    distinct(extid) %>% 
...
# calculate valid test per visit
# calculate invalid test per visit
  # calculate absences per visit (except v7 since this is included in eos section)

  enrolled_tested_v1_2 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V1') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v1_2 <- eff_results %>% 
    filter(visit == 'V1',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v1_2 <- eff_results %>% 
    filter(visit == 'V1',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v2
enrolled_tested_v2_2 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment == 2,
           visit == 'V2') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v2_2 <- eff_results %>% 
    filter(visit == 'V2',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v2_2 <- eff_results %>% 
    filter(visit == 'V2',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v3
  enrolled_tested_v3_2 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V3') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v3_2 <- eff_results %>% 
    filter(visit == 'V3',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v3_2 <- eff_results %>% 
    filter(visit == 'V3',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v4
  enrolled_tested_v4_2 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V4') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v4_2 <- eff_results %>% 
    filter(visit == 'V4',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v4_2 <- eff_results %>% 
    filter(visit == 'V4',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v5  
  enrolled_tested_v5_2 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V5') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v5_2 <- eff_results %>% 
    filter(visit == 'V5',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v5_2 <- eff_results %>% 
    filter(visit == 'V5',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v6
  ...
  enrolled_tested_v6_2 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V6') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v6_2 <- eff_results %>% 
    filter(visit == 'V6',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
  ...
  enrolled_invalid_v6_2 <- eff_results %>% 
    filter(visit == 'V6',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'in',
...
# v7
  enrolled_tested_v7_2 <- eff_results %>% 
    filter(extid %in% enrolled$extid,
           assignment ==2,
           visit == 'V7') %>% 
    distinct(extid) %>% 
  ...
  enrolled_valid_v7_2 <- eff_results %>% 
    filter(visit == 'V7',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'completion',
  ...
  enrolled_invalid_v7_2 <- eff_results %>% 
    filter(visit == 'V7',
           assignment == 2,
           extid %in% enrolled$extid,
           efficacy_status == 'completion',
...
flowchart_2 <- glue("
  digraph flowchart {{
    ...
    preselected [label = '{efficacy_selected_2} pre-selected children'];
    extids_visited [label = '{extid_visited_2} children visited'];
    non_enrolled [label = '{non_enrolled_2} non-enrolled children'];
    non_enrolled_reasons [label = 'Absent {num_not_enrol_absent_2}\nMigrated/Died {num_not_enrol_mig_died_2}\nRefusal {num_not_enrol_ref_2}\nNot eligible {num_not_enrol_eligible_2}\nOther {num_not_enrol_other_2}'];
    enrolled [label = '{enrolled_2} enrolled subjects'];
    v1 [label = '{enrolled_tested_v1_2} visited in V1\nValid test in V1: {enrolled_valid_v1_2}\nInvalid test in V1: {enrolled_invalid_v1_2}\n Absent in V1: {enrol_absent_v1_2}'];
    v2 [label = '{enrolled_tested_v2_2} visited in V2\nValid test in V2: {enrolled_valid_v2_2}\nInvalid test in V2: {enrolled_invalid_v2_2}\n Absent in V2: {enrol_absent_v2_2}'];
    v3 [label = '{enrolled_tested_v3_2} visited in V3\nValid test in V3: {enrolled_valid_v3_2}\nInvalid test in V3: {enrolled_invalid_v3_2}\n Absent in V3: {enrol_absent_v3_2}'];
    v4 [label = '{enrolled_tested_v4_2} visited in V4\nValid test in V4: {enrolled_valid_v4_2}\nInvalid test in V4: {enrolled_invalid_v4_2}\n Absent in V4: {enrol_absent_v4_2}'];
    v5 [label = '{enrolled_tested_v5_2} visited in V5\nValid test in V5: {enrolled_valid_v5_2}\nInvalid test in V5: {enrolled_invalid_v5_2}\n Absent in V5: {enrol_absent_v5_2}'];
    v6 [label = '{enrolled_tested_v6_2} visited in V6\nValid test in V6: {enrolled_valid_v6_2}\nInvalid test in V6: {enrolled_invalid_v6_2}\n Absent in V6: {enrol_absent_v6_2}'];
    v7 [label = '{enrolled_tested_v7_2} visited in V7\nValid test in V7: {enrolled_valid_v7_2}\nInvalid test in V7: {enrolled_invalid_v7_2}\n {enrolled_comp_2} completed the study'];
    eos_fu_v2 [label = 'Early termination in V2: {enrolled_eos_fu_v2_2}'];
    eos_fu_v3 [label = 'Early termination in V3: {enrolled_eos_fu_v3_2}'];
    eos_fu_v4 [label = 'Early termination in V4: {enrolled_eos_fu_v4_2}'];
    eos_fu_v5 [label = 'Early termination in V5: {enrolled_eos_fu_v5_2}'];
    eos_fu_v6 [label = 'Early termination in V6: {enrolled_eos_fu_v6_2}'];
    eos_fu_v7 [label = 'Early termination in V7: {enrolled_eos_fu_v7_2}'];
...
if (any(duplicated(total_efficacy_v7$extid))) {
  ...
  cat("Note: There are no duplicates based on extid in the df.")
}
```
...
df_readable <- reactable(df_investigation, columns = list(), 
                                              highlight = TRUE,
                                              resizable = TRUE,
                                              bordered = TRUE,
                                              striped = TRUE,
