...
in_type_v2 <- efficacy_v2 %>%
  ...
  mutate('Percent' = (as.numeric(`Number of in people`)) / as.numeric(nrow(efficacy_v2)) * 100) %>%
...
eff_rdt_prop_totals_v1 <- eff_results_v1 %>%
  ...
  mutate(Percent = (as.numeric(`Count`)) / as.numeric(nrow(eff_results_v1)) * 100) %>%
  mutate(Percent = round(Percent, 2)) %>%
...
eff_rdt_prop_totals_v2 <- eff_results_v2 %>%
  ...
  summarise('Count' = n()) %>%
  add_row(combination = 'total children', 'Count' = sum(.$'Count')) %>%
  mutate(Percent = (as.numeric(`Count`)) / as.numeric(nrow(eff_results_v2)) * 100) %>%
  mutate(Percent = round(Percent, 2)) %>%
  mutate(Percent = paste0(Percent, '%')) %>%
  rename('Combination type' = combination)
```
