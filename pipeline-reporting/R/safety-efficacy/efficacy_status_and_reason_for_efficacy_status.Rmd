...
eff_cluster_rdt_results <- eff_results %>%
  select(cluster, combination) %>%
  group_by(cluster, combination) %>%
  summarise('Count' = n()) %>%
...
eff_rdt_prop_totals <- eff_results %>%
...
reactable(eff_rdt_prop_totals, columns = list(
...
eff_cluster_rdt_results_readable <- reactable(eff_cluster_rdt_results, columns = list(
  `Combination type` = colDef(align = 'right')
))
