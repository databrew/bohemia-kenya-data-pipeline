...
# missed V2
v2_not_visited <- anti_join(v1_clean, v2safety, by='extid') %>% 
    select(cluster, extid, hhid) %>%
    mutate(visit = 'V2',
           safety_status = 'out',
           safety_reason = 'missed V2',
           starting_safety_status = 'out')
...
# missed V3
v3_not_visited <- anti_join(v2_clean, v3safety, by='extid') %>% 
    select(cluster, extid, hhid) %>%
    mutate(visit = 'V3',
           safety_status = 'out',
           safety_reason = 'missed V3',
           starting_safety_status = 'out')
...
# make a safety v3 target by adding missed extids and removing eos, refusal, migrated and died
  v4_target <- v3_clean %>%
...
Now that we have determined why the target hh and visited hh numbers differ, we will begin to find the V4 target number. To create the V4 target, we will only keep households that have at least one member in. Therefore, we must remove the hh below:
...
* `r num_hhid_v3_total_visited` visited - `r num_hhid_v3_removals` houses without at least 1 person in = `r num_hhid_v3_total_visited - num_hhid_v3_removals` V3 Clean
...
Now that we understand why the target is different, let's clean V3 by removing all people that do not belong to a household with at least one person in: 
...
Now that the data has been cleaned, we will calculate the V4 Target number by adding the people missed in V3 to the V3 Clean. The V4 Target will reflect the number of people who can be visited in the next visit (out and in people).
