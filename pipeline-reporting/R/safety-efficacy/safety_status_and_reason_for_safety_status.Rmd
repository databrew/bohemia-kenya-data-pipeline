...
# make table of safety statuses
  ...
  v4safety_target_statuses <- v3_target %>%
    filter(safety_status %in% c('in', 'out'),
           !safety_reason %in% c('migrated', 'died')) %>%
    group_by(safety_status) %>%
    summarize(`V3 Target` = n()) %>%
  ...
  v4safety_target_calculated_statuses <- v3safety %>%
    bind_rows(v3_not_visited) %>%
    filter(!safety_reason %in% c('migrated', 'died'),
           !extid %in% c(v3_100_refusal_dataset$extid, 
                         v3_death$extid, 
                         v3_migrations$extid)) %>%
    group_by(safety_status) %>%
...
# tracking safety statuses
  v4_status_pathway_data <- v3safety %>%
    mutate(pathway = case_when(
      starting_safety_status == 'out' & safety_status == 'out' ~ 'out to out',
      starting_safety_status == 'eos' & safety_status == 'eos' ~ 'previously eos',
      starting_safety_status == 'refusal' & safety_status == 'refusal' ~ 'previously refusal',
...
Now that we have determined why the target hh and visited hh numbers differ, we will begin to find the V3 target number. To create the V3 target, we must remove the hh below:
...
* `r num_hhid_v3_total_visited` visited - `r num_hhid_v3_100_refusal` 100% refusals = `r num_hhid_v3_total_visited - num_hhid_v3_100_refusal` V1 Clean

Finally, to create the V4 target we want to keep all people who we visited that are not a part of the households removed and we need to include the households that were missed in V3:

* `r num_hhid_v3_total_visited` visited + `r num_hhid_v3_not_visited - num_v3_hhid_visited_extid_not` not visited - `r num_hhid_v3_100_refusal` 100% refusals = `r num_hhid_v3_total_visited + (num_hhid_v3_not_visited - num_v3_hhid_visited_extid_not) - num_hhid_v3_100_refusal` V4 target
...
* OUT: `r num_v3_out` V3 Visited out - `r num_v3_out_died` died - `r num_v3_out_mig` migrated =  `r num_v3_out - num_v3_out_died - num_v3_out_mig` V3 Clean out + `r num_extid_v3_not_visited` missed people = `r num_v4_target_out_start + num_extid_v3_not_visited` OUT (participants who can still enter the study)
