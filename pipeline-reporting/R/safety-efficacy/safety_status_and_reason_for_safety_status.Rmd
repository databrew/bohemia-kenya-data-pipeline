...
# the assignment found below is deprecated and we must use the assignment in safety: 
# 'bohemia-lake-db/bohemia_ext/intervention_assignment.csv
...
# create the dataset for the extid explorer
  data_investigation <- data %>%
    ...
    select(visit, extid, safety_status, safety_reason, 
           ...
           dob_safety_visit, todays_date_v0, todays_date_safety, arm, intervention)
  
# a trial profile tracks where every person went
  by_visit <- data %>%
    pivot_wider(id_cols = c(extid, hhid, cluster, arm, intervention), 
                names_from = visit,
    ...
    select(extid, hhid, safety_status_V1, safety_status_V2, safety_status_V3,
           ...
           safety_reason_V4, arm, intervention, cluster) %>%
...
```{r safety_trial_overall}
 ...
 # a trial profile tracks where every person went
  by_path <- data %>%
    pivot_wider(id_cols = c(extid, hhid, cluster, arm), 
                names_from = visit,
...
# not enrolled participants
  ...
  not_enrolled <- by_visit %>%
    filter(!(extid %in% enrolled$extid),
           !(is.na(arm))) # this removes the 1795 extids that were dropped due to dropping of clusters
...
trial_profile <- data %>%
  ...
  select(extid, visit, enrolled, arm) %>%
  filter(!(is.na(arm))) %>%
    arrange(visit) %>%
    group_by(extid) %>%
    slice(1) %>%
...
# calculate treated, not treated, and visited for each visit and absent
  ...
  enrolled_not_treated_v1_1 <- data %>% 
    filter(extid %in% enrolled$extid,
           arm ==1,
           visit == 'V1',
...
#v2: in V2 you have 295 people who are not eos who have an NA for took_drug because they are absent (adding this makes the total visited subjects)
  ...
  enrolled_not_treated_v2_1 <- data %>% 
    filter(extid %in% enrolled$extid,
           arm ==1,
           visit == 'V2',
           !(safety_status %in% c('eos', 'out')),
...
#v3: in V3 you have 468 people who are not eos who have an NA for took_drug (468 absences)
  ...
  enrolled_not_treated_v3_1 <- data %>% 
    filter(extid %in% enrolled$extid,
           arm ==1,
           visit == 'V3',
           !(safety_status %in% c('eos', 'out')),
...
# calculate treated, not treated, and visited for each visit
  ...
  enrolled_not_treated_v1_2 <- data %>% 
    filter(extid %in% enrolled$extid,
           arm ==2,
           visit == 'V1',
           !(safety_status %in% c('eos', 'out')),
...
#v2: in V2 you have 295 people who are not eos who have an NA for took_drug because they are absent (adding this makes the total visited subjects)
  ...
  enrolled_not_treated_v2_2 <- data %>% 
    filter(extid %in% enrolled$extid,
           arm ==2,
           visit == 'V2',
           !(safety_status %in% c('eos', 'out')),
...
#v3: in V3 you have 468 people who are not eos who have an NA for took_drug (468 absences)
  ...
  enrolled_not_treated_v3_2 <- data %>% 
    filter(extid %in% enrolled$extid,
           arm ==2,
           visit == 'V3',
           !(safety_status %in% c('eos', 'out')),
...
# calculate early termination for V2, V3, V4
  ...
  enrolled_preg_v2_2 <- pregnancy_data_2 %>% 
    filter(visit == 'V2') #12 
  enrolled_preg_v2_2_num <- enrolled_preg_v2_2 %>% 
    distinct(extid) %>% 
    nrow()
  enrolled_preg_v3_2 <- pregnancy_data_2 %>% 
    filter(visit == 'V3') #12 
  enrolled_preg_v3_2_num <- enrolled_preg_v3_2 %>% 
    distinct(extid) %>% 
    nrow()
  enrolled_preg_v4_2 <- pregnancy_data_2 %>% 
    filter(visit == 'V4') #12
