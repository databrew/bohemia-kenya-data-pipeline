...
total_safety_v1 <- total_safety_v1 %>%
...
safety_eos_v1 <- safety_repeat_individual %>%
...
safety_eos_v1 <- left_join(safety_eos_v1, clean_safety, by=c('PARENT_KEY'='KEY')) %>%
  filter(visit == 'V1')
...
safetynew_eos_v1 <- safetynew_repeat_individual %>%
...
safetynew_eos_v1 <- left_join(safetynew_eos_v1, clean_safetynew, by=c('PARENT_KEY' = 'KEY')) %>%
  filter(visit == 'V1')
...
safety_out_v1 <- safety_repeat_individual %>%
...
# need to filter by visit from clean_safety
safety_out_v1 <- left_join(safety_out_v1, clean_safety, by=c('PARENT_KEY'='KEY')) %>%
  filter(visit == 'V1')
...
safetynew_out_v1 <- safetynew_repeat_individual %>%
...
# need to filter by visit from clean_safety
safetynew_out_v1 <- left_join(safetynew_out_v1, clean_safetynew, by=c('PARENT_KEY'='KEY')) %>%
  filter(visit == 'V1')
...
safetynew_out_v1$reason[safetynew_out_v1$obvious_screening == 'Witness'] <- 'no witness'
safetynew_out_v1$reason[safetynew_out_v1$ind_witness_present == 'no'] <- 'no witness'
...
safety_refusal_v1 <- safety_repeat_individual %>%
...
# need to filter by visit from clean_safety
safety_refusal_v1 <- left_join(safety_refusal_v1, clean_safety, by=c('PARENT_KEY'='KEY')) %>%
  filter(visit == 'V1')
...
safetynew_refusal_v1 <- safetynew_repeat_individual %>%
  select('PARENT_KEY', 'extid', 'safety_status', 'obvious_screening_status', 'obvious_screening', 'ind_thumbprint_status', 'ind_sign_icf_status', 'minor_assent_status') %>%
  mutate(type = 'safetynew') %>%
  filter(safety_status == 'refusal')

# need to filter by visit from clean_safety
safetynew_refusal_v1 <- left_join(safetynew_refusal_v1, clean_safetynew, by=c('PARENT_KEY'='KEY')) %>%
  filter(visit == 'V1')
...
ggplot(plot_sex, aes(x = sex, y = count, fill = sex)) +
  geom_bar(stat = 'identity', 
           position = 'dodge',
           show.legend = FALSE) +
