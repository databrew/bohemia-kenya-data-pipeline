...
enrolled_1 <- enrolled %>%
  ...
  enrolled_comp_1 <- data %>% 
    filter(assignment ==1,
           safety_status == 'completion') %>% 
    distinct(extid) %>% 
    nrow()
  
  enrolled_eos_fu_1 <- data %>% 
    filter(extid %in% enrolled$extid,
           assignment == 1, 
           enrolment == 'follow up',
           safety_status == 'eos') %>% 
...
flowchart_1 <- glue("
  digraph flowchart {{
    ...
    reasons [label = '{enrolled_treated_v1_1} treated subjects in V1\n{enrolled_treated_v2_1} treated subjects in V2\n{enrolled_treated_v3_1} treated subjects in V3\n{enrolled_comp_1} completed the study'];
    eos_fu [label = '{enrolled_eos_fu_1} did not complete the study'];
    pregnant [label = '{took_drug_1} women became pregnant (PFU)'];
