...
# create a dataset of enrolled participants (people who became safety status in)
  ...
  enrol_eos_table <- enrol_eos_data %>%
    ...
    distinct(extid) %>% # 95076-02 went in to eos in V2 and V3 making the flowcharts numbers different than the tables which is why this code is here
    summarise('N' = n()) %>%
...
# calculate early termination for V2, V3, V4
  ...
  enrolled_preg_v2_1 <- data %>% 
    filter(assignment == 1,
           visit == 'V2',
           extid %in% enrolled$extid,
           safety_status != 'out',
           pregnancy_status == 'in') #12 
  enrolled_preg_v2_1_num <- enrolled_preg_v2_1 %>% 
    distinct(extid) %>% 
    nrow()
  
  enrolled_preg_v3_1 <- data %>% 
    filter(assignment == 1,
           visit == 'V3',
           extid %in% enrolled$extid,
           safety_status != 'out',
           pregnancy_status == 'in',
           !(extid %in% enrolled_preg_v2_1$extid)) # 26
  enrolled_preg_v3_1_num <- enrolled_preg_v3_1 %>%  
    distinct(extid) %>% 
    nrow()
  
  enrolled_preg_v4_1_num <- data %>% 
    filter(assignment == 1,
           visit == 'V4',
           extid %in% enrolled$extid,
           safety_status != 'out',
           pregnancy_status == 'in',
           !(extid %in% enrolled_preg_v3_1$extid),
           !(extid %in% enrolled_preg_v2_1$extid)) %>% # 48
    distinct(extid) %>% 
    nrow()
...
# calculate early termination for V2, V3, V4
  ...
  # number of women who became pregnant at a follow up visit and took drug in the prior visit
  enrolled_preg_v2_2 <- data %>% 
    filter(assignment == 2,
           visit == 'V2',
           extid %in% enrolled$extid,
           safety_status != 'out',
           pregnancy_status == 'in') #21
  enrolled_preg_v2_2_num <- enrolled_preg_v2_2 %>% 
    distinct(extid) %>% 
    nrow()
  
  enrolled_preg_v3_2 <- data %>% 
    filter(assignment == 2,
           visit == 'V3',
           extid %in% enrolled$extid,
           safety_status != 'out',
           pregnancy_status == 'in') # 49
  enrolled_preg_v3_2_num <- enrolled_preg_v3_2 %>%  
    distinct(extid) %>% 
    nrow()
  
  enrolled_preg_v4_2_num <- data %>% 
    filter(assignment == 2,
           visit == 'V4',
           extid %in% enrolled$extid,
           safety_status != 'out',
           pregnancy_status == 'in',
           !(extid %in% enrolled_preg_v3_2$extid),
           !(extid %in% enrolled_preg_v2_2$extid)) %>% # 18
    distinct(extid) %>% 
    nrow()
