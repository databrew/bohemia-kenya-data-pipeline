...
# create the merged safety data
  clean_safety_repeat <- safety_repeat_individual %>%
    select(PARENT_KEY, extid, safety_status, starting_safety_status, intervention, dob_string, 
           ...
           obvious_screening, baby_not_week_old_eos, accept_preg_test_yn,
           accept_preg_test_yn_short, accept_preg_test_yn_v4, preg_test_2_result, 
           preg_test_2_result_short, preg_test_2_result_v4, pregnant_eos, 
...
clean_safetynew_repeat <- safetynew_repeat_individual %>%
    select(PARENT_KEY, extid, safety_status, dob, sex, participant_take_drug, 
           ...
           baby_not_week_old_eos, accept_preg_test_yn, preg_test_2_result, 
           pregnant_eos, pregnancy_section_eos, 
  ...
  mutate(enrolment = case_when(
    all(safety_status == "out") ~ "never enrolled",
    starting_safety_status == "out" | is.na(starting_safety_status) & safety_status != "out" ~ "enrolment",
    TRUE ~ "follow up")) %>%
  ungroup() %>%
    mutate(cluster = as.numeric(substr(extid, 1, 2)),
           ...
           safety_reason = case_when(
             ...
             preg_test_refuse_eos_short == '1' |
               ...
               pregnancy_section_eos_v4 == '1'|
             accept_preg_test_yn == 'refuse'|
             accept_preg_test_yn_short == 'refuse' |
             accept_preg_test_yn_v4 == 'refuse' |
             preg_test_2_result == 'refuse' | 
             preg_test_2_result_short == 'refuse' |
             preg_test_2_result_v4 == 'refuse' ~ 'pregnancy test refusal',
             pregnant_eos == '1' |
    ...
    # get the correct assignments by cluster 
  left_join(assignment, by='cluster') %>% 
  left_join(assignment_code, by=c('assignment' = 'arm'))
...
# create the dataset for the extid explorer
  data_investigation <- df %>%
    ...
    select(visit, extid, safety_status, safety_reason, 
           ...
           dob_safety_visit, todays_date_v0, todays_date_safety, intervention.x, intervention.y)
  
# a trial profile tracks where every person went
  by_visit <- df %>%
    pivot_wider(id_cols = c(extid, hhid, cluster, intervention.y), 
                names_from = visit,
...
trial_by_visit <- df %>%
  ...
  group_by(visit, intervention.y) %>%
    summarise('N' = n()) %>%
    group_by(visit) %>%
    pivot_wider(names_from = intervention.y, values_from = N) %>%
    ungroup() %>%
    arrange(visit) %>%
...
# create a dataset of enrolled participants (people who became safety status in)
  ...
  enrolled_table <- enrolled_data %>%
    arrange(visit) %>%
    group_by(safety_status, intervention.y) %>%
    summarise('N' = n()) %>%
    group_by(safety_status) %>%
    pivot_wider(names_from = intervention.y, values_from = N) %>%
    ungroup() %>%
  ...
  enrol_eos_table <- enrol_eos_data %>%
    arrange(visit) %>%
    group_by(safety_reason, intervention.y) %>%
  ...
  enrol_comp_table <- df %>%
    ...
    summarise('N' = n()) %>%
    group_by(safety_status) %>%
    pivot_wider(names_from = intervention.y, values_from = N, values_fill = 0) %>%
    ungroup() %>%
...
# not enrolled participants
  ...
  not_enrolled <- by_visit %>%
    filter(!(extid %in% enrolled$extid),
           !(is.na(intervention.y))) # this removes the 1795 extids that were dropped due to dropping of clusters
  ...
  not_enrolled_data <- df %>%
    ...
    pivot_wider(id_cols = c(extid, hhid, cluster, intervention.y),
                ...
                values_from = c(safety_status, safety_reason)) %>%
    group_by(extid) %>%
    mutate(most_recent_status = case_when(

      any(c_across(starts_with("safety_status")) %in% 'eos') ~ 'eos',

      any(c_across(starts_with("safety_status")) == 'refusal') ~ 'refusal',

      any(c_across(starts_with("safety_status")) == 'in') ~ 'in',
      
      any(c_across(starts_with("safety_status")) == 'out') ~ 'out',
      
      TRUE ~ NA_character_ # Default case
    )) %>% 
    mutate(not_enrolled_reason = case_when(

      any(c_across(starts_with("safety_reason")) %in% c('migrated')) ~ 'migrated',
      
      any(c_across(starts_with("safety_reason")) %in% c('died')) ~ 'died',
      
      any(c_across(starts_with("safety_reason")) == 'a person who does not want to participate') ~ 'a person who does not want to participate',
      
      any(c_across(starts_with("safety_reason")) %in% c('does not speak English or Swahili')) ~ 'does not speak English or Swahili',
      
      any(c_across(starts_with("safety_reason")) == 'minor not sign assent') ~ 'minor not sign assent',
      
      any(c_across(starts_with("safety_reason")) == 'not agree or sign informed consent') ~ 'not agree or sign informed consent',
      
      any(c_across(starts_with("safety_reason")) == 'not consented or provided their thumbprint') ~ 'not consented or provided their thumbprint',
      
      any(c_across(starts_with("safety_reason")) %in% c('baby under 1 week')) ~ 'baby under 1 week',
      
      any(c_across(starts_with("safety_reason")) %in% c('concom meds')) ~ 'concom meds',
      
      any(c_across(starts_with("safety_reason")) %in% c('not agree to safety procedures')) ~ 'not agree to safety procedures',
      
      any(c_across(starts_with("safety_reason")) %in% c('not resident')) ~ 'not resident',
      
      any(c_across(starts_with("safety_reason")) %in% c('not wish to continue')) ~ 'not wish to continue',
      
      any(c_across(starts_with("safety_reason")) %in% c('other trials')) ~ 'other trials',
      
      any(c_across(starts_with("safety_reason")) %in% c('participant withdrew informed consent (refuse drug)')) ~ 'participant withdrew informed consent (refuse drug)',
      
      any(c_across(starts_with("safety_reason")) %in% c('spent night at the hospital')) ~ 'spent night at the hospital',
      
      any(c_across(starts_with("safety_reason")) %in% c('study drug')) ~ 'study drug',
      
      any(c_across(starts_with("safety_reason")) %in% c('under weight')) ~ 'under weight',
      
      any(c_across(starts_with("safety_reason")) %in% c('severe illness')) ~ 'severe illness',
      
      any(c_across(starts_with("safety_reason")) %in% c('visited loa loa')) ~ 'visited loa loa',
      
      any(c_across(starts_with("safety_reason")) %in% c('pregnant')) ~ 'pregnant',
      
      any(c_across(starts_with("safety_reason")) %in% c('pregnancy test refusal')) ~ 'pregnancy test refusal',
      
      any(c_across(starts_with("safety_reason")) %in% c('A baby that cannot walk yet')) ~ 'A baby that cannot walk yet',
      
      any(c_across(starts_with("safety_reason")) %in% c('no witness')) & safety_reason_V3 == 'no witness' ~ 'no witness',
      
      any(c_across(starts_with("safety_reason")) == 'absent') ~ 'absent',
      # Default case
      TRUE ~ NA_character_
    ))
  ...
  not_enrolled_table <- not_enrolled_data %>%
    group_by(most_recent_status, intervention.y) %>%
    summarise('N' = n()) %>%
    group_by(most_recent_status) %>%
    pivot_wider(names_from = intervention.y, values_from = N) %>%
    ungroup() %>%
    arrange(most_recent_status) %>%
    rename('NOT ENROLLED status' = most_recent_status) %>%
...
# EOS
  ...
  not_enrol_eos_table <- not_enrolled_data %>%
    filter(most_recent_status == 'eos') %>%
    group_by(not_enrolled_reason, intervention.y) %>%
    summarise('N' = n()) %>%
    group_by(not_enrolled_reason) %>%
    pivot_wider(names_from = intervention.y, values_from = N, values_fill = 0) %>%
    ungroup() %>%
    arrange(not_enrolled_reason) %>%
    rename('EOS reason' = not_enrolled_reason) %>%
    add_row('EOS reason' = 'total visited', 
            'Control' = sum(.$'Control'),
            'Treatment' = sum(.$'Treatment')) %>%
    mutate(Total = `Control` + `Treatment`)
...
# OUT
  ...
  not_enrol_out_table <- not_enrolled_data %>%
    filter(extid %in% not_enrolled$extid,
           most_recent_status == 'out') %>% 
    group_by(not_enrolled_reason, intervention.y) %>%
    summarise('N' = n()) %>%
    group_by(not_enrolled_reason) %>%
    pivot_wider(names_from = intervention.y, values_from = N, values_fill = 0) %>%
    ungroup() %>%
    arrange(not_enrolled_reason) %>%
    rename('OUT reason' = not_enrolled_reason) %>%
...
# REFUSAL
  ...
  not_enrol_ref_table <- not_enrolled_data %>%
    filter(most_recent_status == 'refusal') %>%
    group_by(not_enrolled_reason, intervention.y) %>%
    summarise('N' = n()) %>%
    group_by(not_enrolled_reason) %>%
    pivot_wider(names_from = intervention.y, values_from = N, values_fill = 0) %>%
    ungroup() %>%
    arrange(not_enrolled_reason) %>%
    rename('REFUSAL reason' = not_enrolled_reason) %>%
...
trial_profile <- df %>%
  ...
  select(extid, visit, enrolled, intervention.y) %>%
  filter(!(is.na(intervention.y))) %>%
  group_by(intervention.y) %>%
    distinct(extid, .keep_all = TRUE) #%>%
    # arrange(visit) %>%
    # group_by(extid) %>%
    # slice(1) %>%
    # ungroup()
...
total_trial_table <- trial_profile %>%
    arrange(visit) %>%
    group_by(enrolled, intervention.y) %>%
    summarise('N' = n()) %>%
    group_by(enrolled) %>%
    pivot_wider(names_from = intervention.y, values_from = N, values_fill = 0) %>%
    ungroup() %>%
...
hhid_visited <- df %>%
  ...
  distinct(hhid) %>% 
  tally() %>% 
  pivot_wider(names_from = intervention.y,
                         values_from = n)

hhid_visited_1 <- hhid_visited$Treatment
hhid_visited_2 <- hhid_visited$Control
...
extid_visited_1 <- hhid_visited$Treatment
extid_visited_2 <- hhid_visited$Control


# calculate non-enrolled subjects
non_enrolled <- not_enrolled %>%
  group_by(intervention.y) %>%
  distinct(extid) %>% 
  tally() %>% 
  pivot_wider(names_from = intervention.y,
...
non_enrolled_1 <- non_enrolled$Treatment
non_enrolled_2 <- non_enrolled$Control


# not enrolled: absent, migrated/died, refusal, non eligible, other

not_enrol_ref_swahili_1 <- df %>%
  filter(!extid %in% enrolled$extid) %>%
  pivot_wider(id_cols = c(extid, hhid, cluster, intervention.y),
              names_from = visit,
              values_from = c(safety_status, safety_reason)) %>%
  rowwise() %>%
  filter(any(c_across(starts_with("safety_status")) == "refusal")) %>%
  filter(any(c_across(starts_with("safety_reason")) == "does not speak English or Swahili")) %>%
  ungroup()
not_enrolled_saf <- df %>%
    filter(!extid %in% enrolled$extid) %>%
    pivot_wider(id_cols = c(extid, hhid, cluster, intervention.y),
    ...
    mutate(not_enrolled_reason = case_when(
      ...
      any(c_across(starts_with("safety_reason")) %in% c('migrated', 'died')) ~ 'migrated/died',

      any(c_across(starts_with("safety_status")) == 'refusal') & !(extid %in% not_enrol_ref_swahili_1$extid) ~ 'refusal',

      any(c_across(starts_with("safety_reason")) %in% c('not resident', 'under weight', 'study drug', 'concom meds', 'severe illness', 'baby under 1 week', 'pregnant', 'pregnancy test refusal', 'other trials', 'visited loa loa')) & any(c_across(starts_with('safety_status')) == 'eos') | any(c_across(starts_with('safety_reason')) %in% c('pregnant', 'severe illness', 'baby under 1 week', 'A baby that cannot walk yet')) & any(c_across(starts_with('safety_status')) == 'out') & !(extid %in% not_enrol_ref_swahili_1$extid) ~ 'non eligible',

      any(c_across(starts_with("safety_reason")) %in% c('does not speak English or Swahili', 'participant withdrew informed consent (refuse drug)', 'not agree to safety procedures')) ~ 'other',
      
      any(c_across(starts_with("safety_reason")) %in% c('no witness')) & safety_reason_V3 == 'no witness' ~ 'other',

      any(c_across(starts_with("safety_reason")) == 'absent') ~ 'absent',
      # Default case
      TRUE ~ NA_character_
    )) %>%
    group_by(intervention.y, not_enrolled_reason) %>%
    tally %>%
    mutate(sum = sum(n)) %>%
    pivot_wider(names_from = not_enrolled_reason,
                values_from = n)


num_not_enrol_mig_died_1 <- not_enrolled_saf %>%
  filter(intervention.y == 'Treatment') %>%
  pull('migrated/died')
num_not_enrol_ref_1 <- not_enrolled_saf %>%
  filter(intervention.y == 'Treatment') %>%
  pull('refusal')
num_not_enrol_eligible_1 <- not_enrolled_saf %>%
  filter(intervention.y == 'Treatment') %>%
  pull('non eligible')
num_not_enrol_absent_1 <- not_enrolled_saf %>%
  filter(intervention.y == 'Treatment') %>%
  pull('absent')
num_not_enrol_other_1 <- not_enrolled_saf %>%
  filter(intervention.y == 'Treatment') %>%
  pull('other')

num_not_enrol_mig_died_2 <- not_enrolled_saf %>%
  filter(intervention.y == 'Control') %>%
  pull('migrated/died')
num_not_enrol_ref_2 <- not_enrolled_saf %>%
  filter(intervention.y == 'Control') %>%
  pull('refusal')
num_not_enrol_eligible_2 <- not_enrolled_saf %>%
  filter(intervention.y == 'Control') %>%
  pull('non eligible')
num_not_enrol_absent_2 <- not_enrolled_saf %>%
  filter(intervention.y == 'Control') %>%
  pull('absent')
num_not_enrol_other_2 <- not_enrolled_saf %>%
  filter(intervention.y == 'Control') %>%
  pull('other')


# Enrolled N = people
  safety_enrolled_f <- df %>%
    filter(enrolment == 'enrolment',
           safety_status == 'in')

  safety_enrolled_count_f <- safety_enrolled_f %>%
    group_by(intervention.y) %>%
    distinct(extid) %>%
    tally() %>%
    pivot_wider(names_from = intervention.y,
  ...
  enrolled_2 <- safety_enrolled_count_f$`Control`
  enrolled_1 <- safety_enrolled_count_f$`Treatment`


# Treated V1 (coverage  = X%) thru V3

  treated <- df %>%
    group_by(intervention.y, visit) %>%
    filter(extid %in% enrolled$extid,
           took_drug == 'yes',
           safety_status != 'out') %>%
    tally() %>%
  ...
  enrolled_treated_v1_2 <- treated %>%
    filter(intervention.y == 'Control') %>%
    pull(V1)
  enrolled_treated_v2_2 <- treated %>%
    filter(intervention.y == 'Control') %>%
    pull(V2)
  enrolled_treated_v3_2 <- treated %>%
    filter(intervention.y == 'Control') %>%
    pull(V3)
  enrolled_treated_v1_1 <- treated %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V1)
  enrolled_treated_v2_1 <- treated %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V2)
  enrolled_treated_v3_1 <- treated %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V3)
...
# completed V4
  ...
  enrolled_comp_v4_2 <- completed$Control
  enrolled_comp_v4_1 <- completed$Treatment
...
# visited 
  ...
  enrolled_visited_v1_2 <- enrolled_visited %>%
    filter(intervention.y == 'Control') %>%
    pull(V1)
  enrolled_visited_v2_2 <- enrolled_visited %>%
    filter(intervention.y == 'Control') %>%
    pull(V2)
  enrolled_visited_v3_2 <- enrolled_visited %>%
    filter(intervention.y == 'Control') %>%
    pull(V3)
  enrolled_visited_v4_2 <- enrolled_visited %>%
    filter(intervention.y == 'Control') %>%
    pull(V4)
  enrolled_visited_v1_1 <- enrolled_visited %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V1)
  enrolled_visited_v2_1 <- enrolled_visited %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V2)
  enrolled_visited_v3_1 <- enrolled_visited %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V3)
  enrolled_visited_v4_1 <- enrolled_visited %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V4)
  
  
# not treated
  enrolled_not_treated <- df %>%
    filter(extid %in% enrolled$extid,
  ...
  enrolled_not_treated_v1_2 <- enrolled_not_treated %>%
    filter(intervention.y == 'Control') %>%
    pull(V1)
  enrolled_not_treated_v2_2 <- enrolled_not_treated %>%
    filter(intervention.y == 'Control') %>%
    pull(V2)
  # no one did not take drug in V3
  # enrolled_not_treated_v3_2 <- enrolled_not_treated %>%
  #   filter(intervention.y == 'Control') %>%
  #   pull(V3)
  # enrolled_not_treated_v4_2 <- enrolled_not_treated %>%
  #   filter(intervention.y == 'Control') %>%
  #   pull(V4)
  enrolled_not_treated_v1_1 <- enrolled_not_treated %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V1)
  enrolled_not_treated_v2_1 <- enrolled_not_treated %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V2)
  # no one did not take drug in V3 or V4
  # enrolled_not_treated_v3_1 <- enrolled_not_treated %>%
  #   filter(intervention.y == 'Treatment') %>%
  #   pull(V3)
  # enrolled_not_treated_v4_1 <- enrolled_not_treated %>%
  #   filter(intervention.y == 'Treatment') %>%
  #   pull(V4)
...
# absent
  ...
  enrol_absent_v2_2 <- enrolled_absent %>%
    filter(intervention.y == 'Control') %>%
    pull(V2)
  enrol_absent_v3_2 <- enrolled_absent %>%
    filter(intervention.y == 'Control') %>%
    pull(V3)
  enrol_absent_v4_2 <- enrolled_absent %>%
    filter(intervention.y == 'Control') %>%
    pull(V4)
  enrol_absent_v2_1 <- enrolled_absent %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V2)
  enrol_absent_v3_1 <- enrolled_absent %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V3)
  enrol_absent_v4_1 <- enrolled_absent %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V4)
...
# early end of study
  ...
  enrolled_eos_fu_v2_2 <- safety_eos_f %>%
    filter(intervention.y == 'Control') %>%
    pull(V2)
  enrolled_eos_fu_v3_2 <- safety_eos_f %>%
    filter(intervention.y == 'Control') %>%
    pull(V3)
  enrolled_eos_fu_v4_2 <- safety_eos_f %>%
    filter(intervention.y == 'Control') %>%
    pull(V4)
  enrolled_eos_fu_v2_1 <- safety_eos_f %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V2)
  enrolled_eos_fu_v3_1 <- safety_eos_f %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V3)
  enrolled_eos_fu_v4_1 <- safety_eos_f %>%
    filter(intervention.y == 'Treatment') %>%
    pull(V4)
  
  
# pregnancy info: number of women who became pregnant at a follow up visit and took drug in the prior visit
  
  preg_flowchart <-  df %>%
    filter(extid %in% enrolled$extid,
           safety_status %in% c('completion', 'eos'),
           starting_safety_status != 'eos') %>%
    mutate(preg = case_when(
      extid %in% pfu$extid ~ 'became pregnant',
      TRUE ~ 'other')) %>%
    filter(preg == 'became pregnant') %>% 
    filter(!(safety_status == 'completion' & preg != 'became pregnant')) %>%
    group_by(intervention.y, visit) %>%
    tally() %>%
    pivot_wider(names_from = visit,
                values_from = n)
  ...
  enrolled_preg_v2_2_num <- preg_flowchart %>%
    filter(intervention.y == 'Control') %>%
    pull('V2')
  enrolled_preg_v3_2_num <- preg_flowchart %>%
    filter(intervention.y == 'Control') %>%
    pull('V3')
  enrolled_preg_v4_2_num <- preg_flowchart %>%
    filter(intervention.y == 'Control') %>%
    pull('V4')
  enrolled_preg_v2_1_num <- preg_flowchart %>%
    filter(intervention.y == 'Treatment') %>%
    pull('V2')
  enrolled_preg_v3_1_num <- preg_flowchart %>%
    filter(intervention.y == 'Treatment') %>%
    pull('V3')
  enrolled_preg_v4_1_num <- preg_flowchart %>%
    filter(intervention.y == 'Treatment') %>%
    pull('V4')
```
...
# create flowchart for assignment 1 or the treatment arm
flowchart_1 <- glue("
  digraph flowchart {{
...
# create flowchart for assignment 2 or the control arm
flowchart_2 <- glue("
  digraph flowchart {{
    node [fontname = Helvetica, shape = box, style = filled, color = lightblue];
