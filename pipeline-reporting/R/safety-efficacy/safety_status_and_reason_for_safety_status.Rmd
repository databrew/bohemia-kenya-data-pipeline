...
```{r pregnant}
  ...
  pregnant_completion_v4 <- safety_pregnant_v4 %>%
    ...
    mutate('Percent of completion people' = (as.numeric(`Number of pregnant`)) / as.numeric(nrow(safety_completion_v4)) * 100) %>%
    mutate('Percent of completion people' = round(`Percent of completion people`, 2)) %>%
    mutate('Percent of completion people' = paste0(`Percent of completion people`, '%')) # Replace zero values with NA
```
...
# do note need in for V4 because a person starts as in in V4 but only becomes eos or completion
# however the safety_v4 variable is used to support the rest of the script so we are leaving it
...
# create a dataset with the in efficacy status people from Safety V2 and then joining Safety V3 and V4 (total_safety_v3 and total_safety_v4)
  ...
  safety_v4 <- left_join(safety_v4, total_safety_v3, by=c('extid')) %>%
    ...
    mutate(visit_type = case_when( # create a column that says whether the safety people are follow up or enrolment.
      ...
      v3_safetystatus == 'out' ~ 'enrolment',
      is.na(v3_safetystatus) ~ 'NA in previous visit'
    )) 
...
# create a dataset with the in efficacy status people from Safety V2 and then joining Safety V2 and V3 (total_safety_v2 and total_safety_v3)
  ...
  safety_v3 <- left_join(safety_v3, total_safety_v2, by=c('extid')) %>%
    ...
    mutate(visit_type = case_when( # create a column that says whether the safety people are follow up or enrolment.
      ...
      v2_safetystatus == 'out' ~ 'enrolment',
      is.na(v2_safetystatus) ~ 'NA in previous visit'
    )) 
...
# create a dataset with the in efficacy status people from Safety V2 and then joining Safety V1 and V2 (total_safety_v1 and total_safety_v2)
  ...
  safety_v2 <- left_join(safety_v2, total_safety_v1, by=c('extid')) %>%
    ...
    mutate(visit_type = case_when( # create a column that says whether the safety people are follow up or enrolment.
      ...
      v1_safetystatus == 'out' ~ 'enrolment',
      is.na(v1_safetystatus) ~ 'NA in previous visit'
    ))
