...
# # add the enrolment vs. follow up column
#   enrolled <- total_safety %>%
#     filter(enrolment == 'yes')
#   
#   follow_up <- total_safety %>%
#     filter(extid %in% enrolled$extid) %>%
#     filter(enrolment == 'no',
#            safety_status != 'out',
#            starting_safety_status == 'in')
#   
#   not_yet_enrolled_due <- total_safety %>%
#     filter(enrolment == 'no',
#            ((starting_safety_status == 'out' &
#              safety_status == 'out') & 
#              (safety_reason == 'absent' | 
#                 safety_reason == 'migrated')) |
#              (starting_safety_status == 'in' &
#                 safety_status == 'in' &
#                 person_present == 'no'))
  
  # enrolment = case_when(
  #            extid %in% safetynew_repeat_individual$extid |
  #              (starting_safety_status == 'out' & 
  #               safety_status != 'out' &
  #              (minor_assent_status == '1' |
  #              ind_sign_icf_status == '1' |
  #              ind_thumbprint_status == '1')) ~ 'enrolment',
  #            (is.na(minor_assent_status) |
  #              is.na(ind_sign_icf_status) |
  #              is.na(ind_thumbprint_status)) &
  #              safety_status != 'out' &
  #              person_present == 'yes' ~ 'follow up',
  #            (is.na(minor_assent_status) |
  #              is.na(ind_sign_icf_status) |
  #              is.na(ind_thumbprint_status)) &
  #              safety_status != 'out' &
  #              person_present != 'yes' ~ 'missed visit')
