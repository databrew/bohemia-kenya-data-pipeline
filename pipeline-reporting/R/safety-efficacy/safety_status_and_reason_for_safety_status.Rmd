...
v3_safety_with_missed <- v3safety %>%
...
# hhs with at least one person in
...
v3_out <- v3safety %>%
  filter(safety_status == 'out')
  num_extid_v3_out <- length(unique(v3_out$extid))
  
v3_out_with_missed <- v3_safety_with_missed %>%
  filter(safety_status == 'out') %>%
  anti_join(v3_out, by='extid')
  num_extid_v3_out_with <- length(unique(v3_out_with_missed$extid))
  
v3_out_mig <- v3_out %>%
  filter(safety_reason == 'migrated')
  num_extid_v3_out_mig <- length(unique(v3_out_mig$extid))
  
v3_out_died <- v3_out %>%
  filter(safety_reason == 'died')
  num_extid_v3_out_died <- length(unique(v3_out_died$extid))
  
v3_out_rem <- v3_removals %>%
  filter(safety_status == 'out',
         !safety_reason %in% c('died', 'migrated'))
  num_extid_v3_out_rem <- length(unique(v3_out_rem$extid))

# refusals
v3_ref <- v3safety %>%
  filter(safety_status == 'refusal')
  num_extid_v3_ref <- length(unique(v3_ref$extid))
  
v3_ref_with_missed <- v3_safety_with_missed %>%
  filter(safety_status == 'refusal') %>%
  anti_join(v3_ref, by='extid')
  num_extid_v3_ref_with <- length(unique(v3_ref_with_missed$extid))
  
v3_ref_rem <- v3_removals %>%
  filter(safety_status == 'refusal')
  num_extid_v3_ref_rem <- length(unique(v3_ref_rem$extid))


# migrations 
v3_migrations <- v3_safety_with_missed %>%
  filter(safety_reason == 'migrated')
  num_extid_v3_migrations <- length(unique(v3_migrations$extid))
  num_hhid_v3_migrations <- length(unique(v3_migrations$hhid))

v3_migrated_hhs <- v3_safety_with_missed %>%
    group_by(hhid) %>%
    filter(all(extid %in% v3_migrations$extid),
           safety_reason %in% c('migrated'))
  num_hhid_v3_all_migrations <- length(unique(v3_migrated_hhs$hhid))
  
v3_migrations_kept <- v3_kept %>%
  filter(safety_reason == 'migrated')
  num_extid_v3_migrations_kept <- length(unique(v3_migrations_kept$extid))
  
v3_migrations_rem <- v3_removals %>%
  filter(safety_reason == 'migrated')
  num_extid_v3_migrations_rem <- length(unique(v3_migrations_rem$extid))
  
  
# deaths
v3_death <- v3_safety_with_missed %>%
  filter(safety_reason == 'died')
  num_extid_v3_death <- length(unique(v3_death$extid))
  num_hhid_v3_death <- length(unique(v3_death$hhid))

v3_death_hhs <- v3_safety_with_missed %>%
    group_by(hhid) %>%
    filter(all(extid %in% v3_death$extid),
           safety_reason %in% c('died'))
  num_hhid_v3_all_death <- length(unique(v3_death_hhs$hhid))
  
v3_death_kept <- v3_kept %>%
  filter(safety_reason == 'died')
  num_extid_v3_death_kept <- length(unique(v3_death_kept$extid))
  
v3_death_rem <- v3_removals %>%
  filter(safety_reason == 'died')
  num_extid_v3_death_rem <- length(unique(v3_death_rem$extid))
  
  
  # pregnancies
  v3_pregnancies <- v3_safety_with_missed %>%
    filter(pregnancy_status == 'in',
           !extid %in% v2_pregnancies$extid)
...
# create safety clean by removing deaths, migrations, and 100% refusal households
  v3_clean <- v3_safety_with_missed %>%
    filter(!extid %in% c(v3_removals$extid, 
                         v3_death$extid, 
                         v3_migrations$extid))
...
# make table of safety statuses
  ...
  v4safety_target_calculated_statuses <- v3safety %>%
    bind_rows(v3_not_visited) %>%
    filter(!safety_reason %in% c('migrated', 'died'),
           !extid %in% c(v3_removals$extid, 
                         v3_death$extid, 
                         v3_migrations$extid)) %>%
    group_by(safety_status) %>%
...
* `r num_hhid_v1_total_visited` visited - `r num_hhid_v1_100_refusal` 100% refusals = `r num_hhid_v1_total_visited - num_hhid_v1_100_refusal` V1 Clean
...
However, for calculation purposes there will be a V2 Calculated Target which will include all people on the roster (in, out, refusals, and eos people who did not refuse, migrate, or part of a 100% refusal household):   
...
* `r num_hhid_v2_total_visited` visited - `r num_hhid_v2_100_refusal` 100% refusals = `r num_hhid_v2_total_visited - num_hhid_v2_100_refusal` V1 Clean
...
* `r num_hhid_v2_total_visited` visited + `r num_hhid_v2_not_visited - num_v2_hhid_visited_extid_not` not visited - `r num_hhid_v2_100_refusal` 100% refusals = `r num_hhid_v2_total_visited + (num_hhid_v2_not_visited - num_v2_hhid_visited_extid_not) - num_hhid_v2_100_refusal` V3 target
...
When we dive deeper to confirm that `r num_hhid_v3_target_real - num_hhid_v3_total_visited` household was missed, we find that `r num_hhid_v3_not_visited - num_v3_hhid_visited_extid_not` household was indeed not visited in V3.  
...
Now that we have determined why the target hh and visited hh numbers differ, we will begin to find the V4 target number. To create the V4 target, first we need to add back in the individuals that missed the visit.

* `r num_hhid_v3_total_visited` visited + `r num_hhid_v3_not_visited - num_v3_hhid_visited_extid_not` missed households = `r num_hhid_v3_safety_with_missed`

Next, we will only keep households that have at least one member in. Therefore, we must remove the hh below:

* `r num_hhid_v3_removals` households that did not have at least one person in,  
* `r num_hhid_v3_all_migrations` households had all individuals migrate, and  
* `r num_hhid_v3_all_death` households had all individuals die.
* Total of above = `r num_hhid_v3_removals + num_hhid_v3_all_migrations + num_hhid_v3_all_death`  

This means that `r num_hhid_v3_removals + num_hhid_v3_all_migrations + num_hhid_v3_all_death` households were removed.

The V3 Clean data will then have the following households:

* `r num_hhid_v3_total_visited` visited - `r num_hhid_v3_removals` houses without at least 1 person in `r num_hhid_v3_not_visited - num_v3_hhid_visited_extid_not` not visited = `r num_hhid_v3_total_visited - num_hhid_v3_removals + (num_hhid_v3_not_visited - num_v3_hhid_visited_extid_not)` V3 Clean
...
* `r num_hhid_v3_total_visited` visited + `r num_hhid_v3_not_visited - num_v3_hhid_visited_extid_not` not visited - `r num_hhid_v3_removals` houses with no in people = `r num_hhid_v3_total_visited + (num_hhid_v3_not_visited - num_v3_hhid_visited_extid_not) - num_hhid_v3_removals` V4 target
...
Now that we understand why the target is different, let's add the missing people back into the dataset:

* `r num_extid_v3_og_visited` (OG visited) + `r num_extid_v3_not_visited` (missed people) = `r num_extid_v3_og_visited + num_extid_v3_not_visited`

Next, let's clean V3 by removing all people that do not belong to a household with at least one person in and by removing all deaths and migrations: 

* `r num_extid_v3_removals` households that did not have at least one person in.
* _Of the removals `r num_extid_v3_death_rem` died and `r num_extid_v3_migrations_rem` migrated. However, in total `r num_extid_v3_migrations` individuals migrated and `r num_extid_v3_death` died in V3._

Now when we remove all households that do not have at least one person in, we get a V3 Clean of `r num_extid_v3_clean`.  

* `r num_extid_v3_total_visited` visited - `r num_extid_v3_removals` houses without at least 1 person in `r num_extid_v3_not_visited` not visited - `r num_extid_v3_death_kept` deaths - `r num_extid_v3_migrations_kept` migrations = `r num_extid_v3_total_visited - num_extid_v3_removals + num_extid_v3_not_visited - num_extid_v3_migrations_kept - num_extid_v3_death_kept` V3 Clean 

Now that the data has been cleaned, we will calculate the V4 Target number which will reflect the number of people who can be visited during the next visit (out and in people).

* `r num_v3_in_target` in + `r num_v4_target_out` out = `r num_v3_in_target + num_v4_target_out` V4 Target   

We calculate the V4 Calculated Target by removing migrations, deaths, and 100% refusal households:
...
* OUT: `r num_extid_v3_out` V3 Visited out + `r num_extid_v3_out_with` missed out people - `r num_extid_v3_out_rem` out people apart of households with no one in - `r num_extid_v3_out_died` died - `r num_extid_v3_out_mig` migrated = `r num_extid_v3_out + num_extid_v3_out_with- num_extid_v3_out_rem - num_extid_v3_out_mig - num_extid_v3_out_died`
* REFUSAL: `r num_extid_v3_ref` V3 Visited refusals - `r num_extid_v3_ref_rem` refusal people apart of households with no one in = `r num_extid_v3_ref - num_extid_v3_ref_rem`
...
* These individuals are coming from the `r length(unique(v4_not_visited$hhid)) - length(unique(v4_hhid_visited_extid_not$hhid))` missed households + `r length(unique(v4_hhid_visited_extid_not$hhid))` households which had no Safety submissions (just Safety New Members).
