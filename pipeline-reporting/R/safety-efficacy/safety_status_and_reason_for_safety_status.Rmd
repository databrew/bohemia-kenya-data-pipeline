...
```{r in}
# Safety V3
## table of how many people are in as enrolment and in as follow up in for each visit
# first create a dataset with the in safety status people from SafetyV2 and then joining Safety V1 and V2 (total_safety_v1 and total_safety_v2)
  safety_v3 <- total_safety_v3 %>%
    ...
    select(extid, v3_safetystatus) # get the v3 'in' people
  
  safety_v3 <- left_join(safety_v3, total_safety_v2, by=c('extid')) %>%
    select(extid, safetystatus, v3_safetystatus) %>%
    rename(v2_safetystatus = safetystatus) # join the v3 'in' people with their v2 statuses
  
# create a column that says whether the safety people are follow up or enrolment. This will be determined by classifying as follow up if they were 'in' in V1 and 'in' in V2 AND classify as enrolment if they were 'out' in V1 and 'in' in V2
safety_v3$visit_type[safety_v3$v2_safetystatus == 'in'] <- 'follow up'
safety_v3$visit_type[safety_v3$v2_safetystatus == 'out' | is.na(safety_v3$v2_safetystatus)] <- 'enrolment'

safety_in_v3 <- safety_v3 %>%
  filter(v3_safetystatus == 'in')

# table that shows how many people are follow up and enrolment in V3
# make a table of reasons people are eos
in_type_v3 <- safety_in_v3 %>%
  group_by(visit_type) %>%
  ...
  mutate('Percent' = (as.numeric(`Number of in people`)) / as.numeric(nrow(safety_in_v3)) * 100) %>%
  mutate('Percent' = round(`Percent`, 2)) %>%
  mutate('Percent' = paste0(`Percent`, '%')) # Replace zero values with NA


## table of how many people are in as enrolment and in as follow up in Safety V2
# first create a dataset with the in safety status people from SafetyV2 and then joining Safety V1 and V2 (total_safety_v1 and total_safety_v2)
...
total_safe_eos_v2$visit_type[total_safe_eos_v2$reason == 'eos in V1'] <- 'eos in V1'
...
## V3######################################
safety_eos_v3 <- left_join(safety_eos, clean_safety, by=c('PARENT_KEY'='KEY')) %>%
  filter(visit == 'V3')

# create a reason column
safety_eos_v3$reason[safety_eos_v3$not_continue_eos == '1'] <- 'not wish to continue'
safety_eos_v3$reason[safety_eos_v3$not_agree_safety_procedures_eos == 1] <- 'not agree to safety procedures'
safety_eos_v3$reason[safety_eos_v3$non_resident_eos == 1] <- 'not resident'
safety_eos_v3$reason[safety_eos_v3$weight_eos == 1 |
                       safety_eos_v3$weight_eos_short == 1] <- 'under weight'
safety_eos_v3$reason[safety_eos_v3$study_drug_eos == 1] <- 'study drug'
safety_eos_v3$reason[safety_eos_v3$concom_meds_eos == 1 |
                       safety_eos_v3$concom_meds_eos_short == 1] <- 'concom meds'
safety_eos_v3$reason[safety_eos_v3$severe_illness_eos == 1 |
                       safety_eos_v3$severe_illness_eos_short == 1] <- 'severe illness'
safety_eos_v3$reason[safety_eos_v3$baby_not_week_old_eos == 1] <- 'baby under 1 week'
safety_eos_v3$reason[safety_eos_v3$pregnant_eos == 1 |
                       safety_eos_v3$pregnancy_section_eos == 1 |
                       safety_eos_v3$pregnancy_section_eos_short == 1 |
                       safety_eos_v3$preg_test_pos_eos_short == 1 |
                       safety_eos_v3$preg_test_2_eos_short == 1] <- 'pregnant'
safety_eos_v3$reason[safety_eos_v3$other_trials_eos == 1] <- 'other trials'
safety_eos_v3$reason[safety_eos_v3$loa_loa_eos == 1] <- 'visited loa loa'
safety_eos_v3$reason[safety_eos_v3$sum_night_hospital_eos >= 1] <- 'spent night at the hospital'
safety_eos_v3$reason[safety_eos_v3$preg_test_refuse_eos_short == 1] <- 'pregnancy test refusal'
safety_eos_v3$reason[safety_eos_v3$refuse_drug_eos == 1 |
                       safety_eos_v3$refuse_drug_eos_2 == 1] <- 'participant withdrew informed consent'
safety_eos_v3$reason[safety_eos_v3$person_died_eos == 1] <- 'died'
safety_eos_v3$reason[safety_eos_v3$person_migrated == 1 &
                       safety_eos_v3$starting_safety_status == 'in'] <- 'migrated'
safety_eos_v3$reason[safety_eos_v3$starting_safety_status == 'eos'] <- 'eos in previous visit'


# do the same for safetynew V2
safetynew_eos_v3 <- left_join(safetynew_eos, clean_safetynew, by=c('PARENT_KEY' = 'KEY')) %>%
  filter(visit == 'V3')

# create a reason column
safetynew_eos_v3$reason[safetynew_eos_v3$not_agree_safety_procedures_eos == 1] <- 'not agree to safety procedures'
safetynew_eos_v3$reason[safetynew_eos_v3$non_resident_eos == 1] <- 'not resident'
safetynew_eos_v3$reason[safetynew_eos_v3$weight_eos == 1] <- 'under weight'
safetynew_eos_v3$reason[safetynew_eos_v3$study_drug_eos == 1] <- 'study drug'
safetynew_eos_v3$reason[safetynew_eos_v3$concom_meds_eos == 1] <- 'concom meds'
safetynew_eos_v3$reason[safetynew_eos_v3$severe_illness_eos == 1] <- 'severe illness'
safetynew_eos_v3$reason[safetynew_eos_v3$baby_not_week_old_eos == 1] <- 'baby under 1 week'
safetynew_eos_v3$reason[safetynew_eos_v3$pregnant_eos == 1 |
                          safetynew_eos_v3$pregnancy_section_eos == 1] <- 'pregnant'
safetynew_eos_v3$reason[safetynew_eos_v3$other_trials_eos == 1] <- 'other trials'
safetynew_eos_v3$reason[safetynew_eos_v3$loa_loa_eos == 1] <- 'visited loa loa'
safetynew_eos_v3$reason[safetynew_eos_v3$refuse_drug_eos == 1 |
                          safetynew_eos_v3$refuse_drug_eos_2 == 1] <- 'participant withdrew informed consent'

# join safetynew_eos and safety_eos by extid, safety status, and reason
total_safe_eos_v3 <- bind_rows(safety_eos_v3, safetynew_eos_v3)

# need the visit_type variable from safety_v2 to make two different eos tables (enrolment vs. follow up)
total_safe_eos_v3 <- left_join(total_safe_eos_v3, safety_v3, by=c('extid'))

total_safe_eos_enrolment_v3 <- total_safe_eos_v3 %>%
  filter(visit_type == 'enrolment')
total_safe_eos_followup_v3 <- total_safe_eos_v3 %>%
  filter(visit_type == 'follow up')

# make a table of both enrolment and followup together since they will be split up later
total_safe_eos_v3$visit_type[total_safe_eos_v3$reason == 'eos in previous visit'] <- 'eos in previous visit'

eos_reasons_v3 <- total_safe_eos_v3 %>%
  group_by(visit_type) %>%
  summarise('Number of eos people' = n()) %>%
  rename('Reason for eos' = visit_type) %>%
  add_row('Reason for eos' = 'total visited', 'Number of eos people' = sum(.$'Number of eos people')) %>%
  mutate('Percent of eos' = (as.numeric(`Number of eos people`)) / as.numeric(nrow(total_safe_eos_v3)) * 100) %>%
  mutate('Percent of eos' = round(`Percent of eos`, 2)) %>%
  mutate('Percent of eos' = paste0(`Percent of eos`, '%')) # Replace zero values with NA
