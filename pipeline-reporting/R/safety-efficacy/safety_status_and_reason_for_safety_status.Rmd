...
# identify the repeats in safety and safetynew
errors_safety <- safety %>%
  select(instanceID, hhid, start_time, end_time) %>%
  filter(duplicated(hhid) | duplicated(hhid, fromLast = TRUE))
errors_safetynew <- safetynew %>%
  select(instanceID, hhid, start_time, end_time) %>%
  filter(duplicated(hhid) | duplicated(hhid, fromLast = TRUE))
```
...
absence <- bind_rows(safety_and_safetynew_eos, safety_and_safetynew_out, safety_and_safetynew_refusal) %>%
  select(extid, safety_status, visit, reason) %>%
  filter(reason == 'absent') %>%
  mutate(group = (ifelse(absence$extid %in% absence1stattempt$extid_enter, 'absent with form', 'absent w/out form')))
...
dist_table <- dist_age %>%
  mutate(age = round(as.numeric(difftime(todays_date, dob, units = "days")) / 365.25, 2)) %>%
  select(extid, sex, age)

# make a histogram of age
hist(dist_table$age, main = "Age Distribution", xlab = "Age")

# make a bar plot of sex
barplot(table(dist_table$sex), main = "Sex Distribution", xlab = "Sex", ylab = "Count")
# need to edit these graphs a bit and then make them appear in RMarkdown and then push code :) 
