...
# calculate early termination for V2, V3, V4
  ...
  # Define a function to extract numeric part from visit strings
  extract_numeric <- function(visit_string) {
    as.numeric(sub("V", "", visit_string))
  }
  
  enrolled_preg_1$visit_numeric <- extract_numeric(enrolled_preg_1$visit)
  
  took_drug_1 <- data %>% 
  filter(extid %in% enrolled_preg_1$extid,
         extract_numeric(visit) == enrolled_preg_1$visit_numeric - 1,
         took_drug == 'yes') %>% 
    distinct(extid) %>% 
    nrow()
...
enrolled_2 <- enrolled %>%
  ...
  enrolled_comp_2 <- data %>% 
    filter(assignment ==2,
           safety_status == 'completion') %>% 
    distinct(extid) %>% 
    nrow()
  
  enrolled_eos_fu_2 <- data %>% 
    filter(extid %in% enrolled$extid,
           assignment == 2, 
           enrolment == 'follow up',
           safety_status == 'eos') %>% 
  ...
  enrolled_preg_2$visit_numeric <- extract_numeric(enrolled_preg_2$visit)
  
  took_drug_2 <- data %>% 
  filter(extid %in% enrolled_preg_2$extid,
         extract_numeric(visit) == enrolled_preg_2$visit_numeric - 1,
         took_drug == 'yes') %>% 
    distinct(extid) %>% 
    nrow()
...
flowchart_2 <- glue("
  digraph flowchart {{
    ...
    reasons [label = '{enrolled_treated_v1_2} treated subjects in V1\n{enrolled_treated_v2_2} treated subjects in V2\n{enrolled_treated_v3_2} treated subjects in V3\n{enrolled_comp_2} completed the study'];
    eos_fu [label = '{enrolled_eos_fu_2} did not complete the study'];
    pregnant [label = '{took_drug_2} women became pregnant (PFU)'];
