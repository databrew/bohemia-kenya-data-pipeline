...
v1safety <- data %>%
  filter(visit == 'V1') %>%
  select(visit, extid, hhid, cluster, safety_status, safety_reason, assignment, type)
...
# make table of safety statuses
  ...
  v1statuses <- merge(v1safety_statuses, v1safety_clean_statuses, by='safety_status')
...
* Individual level: `r num_extid_v0_total_visited` visited households - `r num_v0_removed_extid` removed households = `r num_extid_v0_total_visited - num_v0_removed_extid` target households
...
When we dive deeper to confirm that `r num_hhid_v0_clean - num_hhid_v1_total_visited` households were missed, we find that `r num_hhid_v1_not_visited - num_v1_hhid_visited_extid_not` households were indeed not visited in V1, but interestingly we also find that there were `r num_v1_hhid_visited_extid_not` households in V1 that were visited but at least one member of that household had no data submitted for that visit. 

Now that we have determined why the target hh and visited hh numbers differ, we will begin to find the V2 target number. To create the V2 target, we must remove all hhs with a 100% refusal rate. __A 100% refusal rate houshold is defined as a household that is completely made up of individuals who refused, migrated, or died.__ See below:
...
This means that `r num_hhid_v1_ref_ref + num_hhid_v1_all_migrations + num_hhid_v1_all_death + num_hhid_v1_ref_mig_died` households were removed due to being 100% refusal.

Finally, to create the V2 target we want to keep all people who we visited that are not a part of the 100% refusal households and we need to include the people who were missed in V1:
...
#### Individual Level
Safety V1 visited people who were also visited in V0 (type OG) but also visits new people (type new). In total `r num_extid_v1_total_visited` people were visited in Safety V1 of which `r num_extid_v1_new_visited` were new and `r num_extid_v1_og_visited` were OG. 
...
Now that we understand why the target is different, let's find the target for V2 by removing all migrations, deaths, and 100% refusal households: 
...
_NOTE: the 100% refusal households includes people who migrated and died. Therefore, to prevent people from being counted twice, people who migrated or died and are part of a 100% refusal household are reported in the 100% refusal household number above._ 

* Total of above = `r num_extid_v1_mig_hhs_not_100 + num_extid_v1_died_hhs_not_100 + num_extid_v1_100_refusal` people

Now when we remove migrations, deaths, and people in 100% refusal households and add the people we missed, we get a V2 target of `r num_extid_v1_clean`.

`r num_extid_v1_total_visited` visited + `r num_extid_v1_not_visited` missed - `r num_extid_v1_mig_hhs_not_100 + num_extid_v1_died_hhs_not_100 + num_extid_v1_100_refusal` removed = `r num_extid_v1_total_visited + num_extid_v1_not_visited - (num_extid_v1_mig_hhs_not_100 + num_extid_v1_died_hhs_not_100 + num_extid_v1_100_refusal)` V2 target

To double check everything that has been done, the V1 visited people's safety statuses are compared to the V2 target below: 

```{r v1_safety_statuses}
knitr::kable(v1statuses)
```

* OUT: `r num_v1_out` - `r num_v1_out_mig` migrated - `r num_v1_out_died` died + `r num_extid_v1_not_visited` missed people = `r num_v1_out - num_v1_out_mig - num_v1_out_died + num_extid_v1_not_visited` OUT
* REFUSAL: `r num_v1_refusal` - `r num_v1_ref_removed` people in 100% removal households = `r num_v1_refusal - num_v1_ref_removed` REFUSAL
