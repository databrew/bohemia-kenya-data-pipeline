...
### Table of individual safety statuses
```{r total}
...
extid_and_safety_status_df <- left_join(clean_safety, clean_safetynew, by=c('extid'))
...
safety_eos$reason[safety_eos$safety_inclusion_eos == 1] <- 'NA'
...
safety_out <- safety_repeat_individual %>%
  select('extid', 'safety_status', 'person_out_died', 'person_out_migrated', 'obvious_screening_status', 'obvious_screening') %>%
  mutate(type = 'safety') %>%
...
safety_out$reason[safety_out$obvious_screening == 'Refusal'] <- 'a person who does not want to participate'
...
safety_out$reason[safety_out$obvious_screening == 'Language'] <- 'does not speak English or Swahili'
safety_out$reason[safety_out$obvious_screening == 'Witness'] <- 'no witness'
safety_out$reason[safety_out$extid %in% absence1stattempt$extid] <- 'absent first attempt'
# create a new df for safetynew for out individuals with the reason
safetynew_out <- safetynew_repeat_individual %>%
  select('extid', 'safety_status', 'obvious_screening_status', 'obvious_screening') %>%
  mutate(type = 'safetynew') %>%
  filter(safety_status == 'out')
safetynew_out$reason[safetynew_out$obvious_screening == 'Refusal'] <- 'a person who does not want to participate'
...
safetynew_out$reason[safetynew_out$obvious_screening == 'Language'] <- 'does not speak English or Swahili'
safetynew_out$reason[safetynew_out$obvious_screening == 'Witness'] <- 'no witness'
safetynew_out$reason[safetynew_out$extid %in% absence1stattempt$extid] <- 'absent first attempt'
