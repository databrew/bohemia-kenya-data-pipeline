...
total_efficacy_status <- total_efficacy_v1 %>%
  ...
  summarise('Number of people visited in V1 (target: ?)' = n()) %>%
  arrange(efficacy_status) %>%
  rename('Efficacy status' = efficacy_status) %>%
  add_row('Efficacy status' = 'total people visited', 'Number of people visited in V1 (target: ?)' = sum(.$'Number of people visited in V1 (target: ?)')) %>%
  mutate('Percent complete' = (as.numeric(`Number of people visited in V1 (target: ?)`)) / as.numeric(nrow(total_efficacy_v1)) * 100) %>%
  mutate('Percent complete' = round(`Percent complete`, 2)) %>%
  mutate('Percent complete' = paste0(`Percent complete`, '%'))
reactable(total_efficacy_v1, columns = list(
...
efficacy_eos$reason[efficacy_eos$not_agree_efficacy_procedures_eos == 1] <- 'decline at informed consent/assent'
...
names_to_check <- c('lost to follow up', 'died and was in safety', 'died and was in efficacy', 'migrated', 'absent twice', 'decline participation', 'decline at informed consent/assent', 'not a resident', 'enrolled in other trials')
...
if (length(not_found_names) > 0) {
  message(paste("Note: The reasons like", paste0(not_found_names, collapse = ", "), "is not shown because it is not a reason for anyone being missing."))
}
```
...
efficacy_out$reason[efficacy_out$starting_efficacy_status == 'out' & 
                      efficacy_out$person_absent == '1' & 
                      efficacy_out$eos != 1 & 
                      (efficacy_out$visit == 'V1' | efficacy_out$visit == 'V2' | efficacy_out$visit == 'V3')] <- 'not enrolled in V1, V2, or V3'
...
# because the table is organised by extid and not reason, some reasons may not appear on the table; therefore the next part will create a line of code that generates a message that says which reason is not appearing. In this by names I mean the reason column.
# Specify the names you want to check for
names_to_check <- c('migrated', 'died', 'not enrolled in V1, V2, or V3')
...
efficacy_refusal <- efficacy %>%
  select(KEY, extid, visit, starting_efficacy_status, thumbprint_status, sign_icf_status, minor_assent_status, person_absent) %>%
...
efficacy_refusal$reason[efficacy_refusal$thumbprint_status == 0 |
                          efficacy_refusal$sign_icf_status == 0 |
                          efficacy_refusal$minor_assent_status == 0] <- 'decline at informed consent/assent'
...
refusal_reasons <- efficacy_refusal %>%
  ...
  rename('Reason for eos' = reason) %>%
  add_row('Reason for eos' = 'total visited', 'Number of eos people' = sum(.$'Number of eos people')) %>%
  mutate('Percent complete' = (as.numeric(`Number of eos people`)) / as.numeric(nrow(efficacy_refusal)) * 100) %>%
  mutate('Percent complete' = round(`Percent complete`, 2)) %>%
  mutate('Percent complete' = paste0(`Percent complete`, '%')) # Replace zero values with NA
...
# because the table is organised by extid and not reason, some reasons may not appear on the table; therefore the next part will create a line of code that generates a message that says which reason is not appearing. In this by names I mean the reason column.
# Specify the names you want to check for
names_to_check <- c('declined at informed consent/assent')
