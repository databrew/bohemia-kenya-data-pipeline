...
monitoring1 <- monitoring0 %>%
  dplyr::select(`Cluster` = cluster_number,
                ...
                `Date of consent on screening form` = todays_date,
                `Collection Method` = site,
                `Active or Withdrawn` = active_or_withdrawn,
                `Date of Withdrawal on the screening form` = withdrawal_date)
...
```{r}
monitoring_icf <- monitoring0 %>%
  dplyr::select(`Cluster` = cluster_number,
                `Household ID / Livestock Enclosure ID` = id,
                `Date of consent on screening form` = todays_date) %>%
  dplyr::mutate(
    `Name of Person Receiving in the Field` = NA_character_,
    `Date of Field Receival` = NA_character_,
    `Name of Person Receiving in the Office` = NA_character_,
    `Date of Office Receival` = NA_character_,
    `Name of Archivist Receiving` = NA_character_,
    `Date of Archivist Receival` = NA_character_,
  )

render_report_table(
  monitoring_icf, 
  element_id = "ento-download-icf-list",
  output_filename = "report_ento_monitoring_icf_list.csv")
```
