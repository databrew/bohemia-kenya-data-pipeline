...
tryCatch({
  ...
  cloudbrewr::aws_login(
    ...
    profile_name =  'databrew-sefa.boafo',
...
reactable(light_trap_df, filterable = TRUE)
```
...
reactable(parity_n_dissection, filterable = TRUE)
...
entorc_df <- entorcmorphid[names(entorcmorphid) %>% stringr::str_detect("SubmissionDate|instance_label|cluster|instanceID|todays_date|repeat_tubes_|repeat_tube_")]%>%
  tidyr::pivot_longer(!c(SubmissionDate,todays_date,instance_label,instanceID), names_to = "temp_value", values_to = "value")%>%
  dplyr::mutate(species = stringr::str_extract(temp_value,'gambiae|funestus'),
                KEY_ = gsub("_count","",temp_value),
                KEY = as.character(stringr::str_glue('{instanceID}/group_anopheles_{species}/{KEY_}')),
                num_samp = ifelse(is.na(value),1,value))
...
entorcmorpid_tubes <- rc_df %>% 
  ...
  select(instance_label,
         ...
         `Number of mosquitos in tube` = num_stored)%>%
  left_join(entorcfield %>% select(`Collection date` = todays_date,`Collection location` = hh_or_pit,hhid)->a, by = c("instance_label"="hhid", "todays_date" = "Collection date"))%>%
...
reactable(rc_tubes, filterable = TRUE)
```
