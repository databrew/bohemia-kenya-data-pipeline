...
- Please reach out to Bohemia-Kenya #data-ops for bug reports
...
v0demography <- cloudbrewr::aws_s3_get_table(
  ...
  key = input_key$v0) %>% mutate(across(matches("date"), lubridate::date))
...
light_trap_df <- entoltmorpid_tubes %>%
  ...
  left_join(entoltfield %>% 
              dplyr::select(todays_date,site,instance_label,trap_success) %>%
              dplyr::mutate(todays_date = lubridate::date(todays_date)), 
...
df <- entoltmorphid %>%
  ...
  left_join(entohhfirstvisitke %>%
              ...
              mutate(instance_label = as.character(instance_label),
                     ...
                     roof = stringr::str_c(roof,roof_specify, sep =",")), by = c("site_id"="instance_label"))%>%
  left_join(entolefirstvisitke %>% select(instance_label,
                                          ...
                                          `LE cover` = wall,
                                          `LE fence`=fence,
                                          `LE screened` = screened)%>%mutate(`Animal species kept in LE` = stringr::str_c(animals,animals_other, sep=",")), 
            by = c("site_id"="instance_label"))%>%
  left_join(entoltfield %>% select(instance_label,todays_date,trap_success), by = c("todays_date", "site_id"="instance_label"))%>%
  select(-c(todays_date,wall_specify,roof_specify))%>%
...
entorcmorpid_tubes <- rc_df %>% 
  ...
  select(instance_label,
         ...
         `Number of mosquitos in tube` = num_stored)%>%
  left_join(entorcfield %>% select(`Collection date` = todays_date,`Collection location` = hh_or_pit,hhid), by = c("instance_label"="hhid", "todays_date" = "Collection date"))%>%
...
df <- entorcmorphid %>%
  ...
  left_join(entohhfirstvisitke %>%
              ...
              mutate(instance_label = as.character(instance_label),
                     wall = stringr::str_c(wall,wall_specify, sep = ","),
                     roof = stringr::str_c(roof,roof_specify, sep =",")), by = c("site_id"="instance_label"))%>%
  left_join(entorcfield %>% select(hhid,todays_date,trap_success)%>%mutate(hhid = as.character(hhid)), by = c("site_id"="hhid"))%>%
  select(-c(todays_date,wall_specify,roof_specify))%>%
  select(`collection site`,
         site_id,
         cluster,
