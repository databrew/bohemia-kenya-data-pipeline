...
anomalies_list$visit_already_in_dataset <- pfu %>%
  dplyr::group_by(visit, hhid) %>%
  dplyr::mutate(n = n(),
                key_list = paste0(KEY, collapse = ',')) %>%
  dplyr::filter(n > 1) %>%
  dplyr::mutate(form_id = 'pfu',
                anomalies_id = glue::glue('extid_visit_already_in_dataset'),
                anomalies_description = glue::glue('extid:{hhid} visit:{visit} already in dataset, please check these keys {key_list}'),
...
anomalies_list$detected_multiple_clusters_hh <- pfu %>%
  dplyr::group_by(hhid) %>%
